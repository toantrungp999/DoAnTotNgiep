        ...
        ? notifications.map((notification, index) => {
            ...
            switch (notification.action) {
              case REPLIED_COMMENT:
                action = {
                  url: 'detailProductScreen',
                  data: {
                    _id: notification.target.product._id,
                    title: notification.target.product.name,
                  },
              ...
              case APPROVAL:
                action = {
                  url: 'orderStackScreen',
                  data: {screen: 'orderScreen'},
                  name: 'Người quản lí',
                ...
                <TouchableOpacity
                  style={
                    notification.check === true
                      ? styles.notify
                      : styles.notifyNotCheck
                  }
                  key={notification._id}
                  index={index}
                  onPress={() => {
                    this.props.updateNotification(notification._id);
                    this.props.navigation.push(action.url, action.data);
                  }}>
                  <View style={{width: '23%'}}>
                    <Image
                      style={styles.image}
                      source={{uri: notification.performedBy.image}}
                    />
                  </View>
                  <View style={{width: '77%'}}>
                    <View style={styles.row}>
                      <Text style={styles.textBold}>{action.name}</Text>
                      <Text style={styles.text}>{action.content}</Text>
                    </View>
                    <View style={styles.row}>
                      <Text style={styles.time}>
                        {time_ago(notification.date)}
                      </Text>
                    </View>
                  </View>
                </TouchableOpacity>
...
const mapDispatchToProps = dispatch => ({
  fectchNotifications: (pageSize, page) =>
    dispatch(fectchNotificationsRequest(pageSize, page)),
