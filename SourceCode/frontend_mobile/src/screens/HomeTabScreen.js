...
class HomeTabScreen extends Component {
  ...
  fectchNotifications = () => {
    ...
    if (userInfo) {
      ...
      if (!loading && !feedNews) this.props.fectchNotifications(7);
    }
  };
    ...
    if (userInfo) {
      if (messengers) {
        for (let i = messengers.length - 1; i >= 0; i--) {
          let currentUserSend =
            messengers[i].user1._id === userInfo._id ? 'user1' : 'user2';
          let isMeSend =
            currentUserSend ===
            messengers[i].messages[messengers[i].messages.length - 1].sender;
          if (!isMeSend && messengers[i].check === false) notSeen++;
        }
      }

      if (carts) lengthCarts = carts.length;
    }
    ...
    return (
      <Tab.Navigator
        ...
        {userInfo ? (
          <Tab.Screen
            name="Giỏ hàng"
            options={lengthCarts > 0 ? {tabBarBadge: lengthCarts} : null}
        ...
        {userInfo ? (
          <Tab.Screen
            name="Nhắn tin"
            options={notSeen > 0 ? {tabBarBadge: notSeen} : null}
            component={ListMessengerScreen}
          />
        ) : null}
        {userInfo ? (
          <Tab.Screen
            name="Thông báo"
            options={count > 0 ? {tabBarBadge: count} : null}
            component={NotificationsScreen}
          />
        ) : null}
        <Tab.Screen
          name={userInfo ? 'Cá nhân' : 'Đăng nhập'}
...
const mapDispatchToProps = (dispatch, props) => {
  return {
    ...
    fectchNotifications: pageSize =>
      dispatch(fectchNewNotificationsRequest(pageSize, 1)),
  };
};
