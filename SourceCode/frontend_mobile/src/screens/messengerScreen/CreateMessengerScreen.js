...
const bot = {
  ...
  name: 'Trợ lý ảo',
  image:
    'http://res.cloudinary.com/websitebandienthoai/image/upload/v1621843345/blog/2021-05-24T08:02:24.316Z.jpg',
};
  ...
  render() {
    ...
    userElements.unshift(
      <View key={bot._id} style={styles.row}>
        <Image
          style={{width: 36, height: 36, borderRadius: 18}}
          source={{uri: bot.image}}></Image>
        <TouchableOpacity
          style={{marginLeft: 15}}
          onPress={() => {
            this.onChoose(bot);
          }}>
          <Text style={{fontSize: 16}}>{bot.name}</Text>
        </TouchableOpacity>
      </View>,
    );
    ...
    if (userElements && userInfo.role === CUSTOMER) {
      userElements.unshift(
        <View key={customerCare._id} style={styles.row}>
          <Image
            style={{width: 36, height: 36, borderRadius: 18}}
            source={require('../../../assets/CSKH.jpg')}></Image>
          <TouchableOpacity
            style={{marginLeft: 15}}
            onPress={() => {
              this.onChoose(customerCare);
            }}>
            <Text style={{fontSize: 16}}>{customerCare.name}</Text>
          </TouchableOpacity>
        </View>,
      );
    }
