...
export default class AddRate extends Component {
    ...
    onSubmit = () => {
        ...
        if (content) {
            this.props.onCreateRate({ content, rate }, { _id: this.props.userInfo._id, name: this.props.userInfo.name, image: this.props.userInfo.image, role: this.props.userInfo.role });
            this.setState({
                content: '',
                rate: 5
            });
    ...
    render() {
        ...
        for (let i = 0; i < MAX_RATING; i++) {
            rangeView.push(<TouchableOpacity onPress={() => { this.onChange('rate', i + 1) }} key={i} id={i}><Icon
        ...
        return (
            <View style={styles.row}>
                ...
                <View style={styles.viewSend}>
                    <TouchableOpacity onPress={this.onSubmit}><Text style={styles.sendAddReteBtn}>Đánh giá</Text></TouchableOpacity>
                </View>
            </View>
        );
    }
