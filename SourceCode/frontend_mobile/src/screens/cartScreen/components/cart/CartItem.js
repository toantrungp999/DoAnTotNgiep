...
class CartItem extends Component {
  ...
  onUp = () => {
    ...
    if (quantity < quantityInStore && quantity < 5) {
      ...
      this.showMessage(
        'Cảnh báo',
        'Số lượng sản phẩm vượt quá số lượng có trong kho',
      );
    else
      this.showMessage(
        'Cảnh báo',
  ...
  onDown = () => {
    ...
    if (this.props.quantity === 1) this.prepareToDelete();
  ...
  prepareToDelete = () => {
    console.log('sfsd');
    Alert.alert('Cảnh báo', 'Bạn muốn sản phẩm này khỏi giỏ hàng', [
      {
        text: 'Đồng ý',
  ...
  componentDidUpdate(prevProps) {
    if (
      ...
      this.props.updateLoading !== prevProps.updateLoading
    ) {
      ...
      <View style={styles.container}>
        ...
        <View
          style={[
            ...
            this.props.index === 0 && styles.first,
          ]}>
          ...
          <View style={styles.imageConatiner}>
            <Image
              style={this.props.isVisible ? styles.imageLowColor : styles.image}
                ...
                <View
                  style={
                    this.props.isVisible
                      ? styles.categoryLowColor
                      : styles.category
                  }>
                  <Text>
                    {this.props.type} <AntDesign name="caretdown" size={14} />
                  </Text>
                </View>
              </TouchableOpacity>
            </View>
            <View style={styles.priceSection}>
              {this.props.saleOff !== 0 ? (
                <Text style={styles.salePrice}>
                  {convertNumberToVND(this.props.price)} ₫
                </Text>
              ) : null}
              <Text style={styles.price}>
                {convertNumberToVND(this.props.price - this.props.saleOff)} ₫
              </Text>
            </View>
            <View style={styles.rowItemCenter}>
              <TouchableOpacity onPress={this.onDown}>
                <Text style={styles.btnSubAdd}>-</Text>
              </TouchableOpacity>
              ...
              <View style={styles.deleteContainer}>
                <TouchableOpacity onPress={this.prepareToDelete}>
                  <Ionicons style={styles.delete} name="trash" size={20} />
                </TouchableOpacity>
              </View>
            </View>
            <View style={styles.rowItemCenter}>
              <Text style={styles.quantityInStore}>
                Kho: {this.props.quantityInStore}
              </Text>
              {this.props.quantity > this.props.quantityInStore && (
                <Text style={styles.quantityMessage}>(Không đủ số lượng)</Text>
              )}
            </View>
          </View>
        </View>
