  ...
  checkValidate = (name, value) => {
    ...
    switch (name) {
      case 'password':
        ...
        else if (value.length === 0) errors.password = 'Chưa nhập mật khẩu';
        else errors.password = '';
        break;
      case 'phoneNumber':
        if (value.length === 0) errors.phoneNumber = 'Chưa nhập số điện thoại';
        else {
          checkPhoneNumber = isPhoneNumber(value);
          errors.phoneNumber = !checkPhoneNumber
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      ...
      if (changePhoneSuccess === true)
        Alert.alert('Thông báo', 'Cập nhật số điện thoại thành công');
    }
  }
        ...
        <Background style={styles.containerMain}>
          ...
          <View style={styles.row}>
            <Text style={{color: 'red'}}>
              {changePhoneMessage ? changePhoneMessage : ''}
            </Text>
          </View>
          {loading ? (
