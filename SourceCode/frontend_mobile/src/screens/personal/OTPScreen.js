...
class OTPScreen extends Component {
      ...
      case 'code':
        value.error =
          value.value.length > 0
            ? isValidLength(value.value, 6, 6).error
            : 'Chưa nhập';
  ...
  componentDidUpdate(prevProps) {
    if (
      ...
      if (
        !this.props.userForgotPasswordReducer ||
        this.props.userForgotPasswordReducer.timeout === 0
      ) {
  ...
  verifyOTP = () => {
    if (this.state.code.error) return;
  ...
  sendResetPasswordOTP = () => {
    if (this.state.email) this.props.forgotPassword(this.state.email);
    else this.props.navigation.navigate('ForgotPasswordScreen');
  };
    ...
    return (
      <Background>
        ...
        <TextInput
          ...
          description={
            msgSuccess ||
            'Bạn sẽ nhận mã OTP thông qua số điện thoại. Mã OTP có hiệu lực trong 5 phút.'
          }
        />
        {message ? (
          <View>
        ...
        {loading === true ? null : (
          <TouchableOpacity
            ...
            <Text style={{textAlign: 'right', color: 'blue'}}>Gửi lại OTP</Text>
          </TouchableOpacity>
        )}
...
const mapDispatchToProps = (dispatch, props) => {
  return {
    verifyOTP: code => {
      dispatch(verifyOTPRequest(code));
    },
