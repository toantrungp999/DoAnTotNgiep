...
async function getMessengers(userId, isAdmin) {
    if (!userId)
        throw new Error("userId must has value!");
    //await Messenger.deleteMany();
    return await Messenger.find({ $or: !isAdmin ? [{ user1: userId }, { user2: userId }] : [{ user1: userId }, { user2: userId }, { user1: null }, { user2: null }] })
        .sort([['date', -1]])
...
async function updateMessengerCheck(messengerId) {
    ...
    await Messenger.updateOne({ _id: messengerId },
        { $set: { check: true } });
...
async function sendMessageToUser(sendBy, to, content) {
    ...
    if (!content)
        throw new Error("content must has value!");
    const checkMessengerExist = await Messenger.findOne({ $or: [{ user1: sendBy, user2: to }, { user1: to, user2: sendBy }] });
    ...
    if (checkMessengerExist === null) {
        ...
        message = { sender: checkMessengerExist.user1 == sendBy ? "user1" : "user2", type: 'msg', content: content };

        await Messenger.updateOne({ _id: checkMessengerExist._id }, {
            message: message,
            date: dateNow,
            check: false
        });
...
async function SendMessageToBot(message, userId) {
    const response = await apiCaller("POST", process.env.URL_CHAT_BOT, { message });
        ...
        switch (data.type) {
            case Types.ASK_PRODUCT:
                ...
                if (tmp) {
                    ...
                    const quantityOptions = await QuantityOption.find({ productId: product._id })
                    ...
                    if (data.data.color && !data.data.size.trim()) {
                        for (let i = quantityOptions.length - 1; i >= 0; i--)
                            if (!Extention.stringToSlug(quantityOptions[i].colorId.color).includes(data.data.color.trim()))
                                quantityOptions.splice(i, 1);
                    }
                    else if (data.data.size && !data.data.color.trim()) {
                        for (let i = quantityOptions.length - 1; i >= 0; i--)
                            if (!Extention.stringToSlug(quantityOptions[i].colorId.color).includes(data.data.size.trim()))
                                quantityOptions.splice(i, 1);
                    }
                    else if (data.data.size && data.data.color) {
                        for (let i = quantityOptions.length - 1; i >= 0; i--)
                            if (!Extention.stringToSlug(quantityOptions[i].colorId.color).includes(data.data.color.trim()) || !Extention.stringToSlug(quantityOptions[i].sizeId.size).includes(data.data.size.trim()))
                                quantityOptions.splice(i, 1);
                    }
                    ...
                    for (let i = quantityOptions.length - 1; i >= 0; i--) {
                        if (quantityOptions[i].quantity !== 0) {
                            _quantityOptions.push({ _id: quantityOptions[i]._id, quantity: quantityOptions[i].quantity, colorId: quantityOptions[i].colorId._id, color: quantityOptions[i].colorId.color, sizeId: quantityOptions[i].sizeId._id, size: quantityOptions[i].sizeId.size });
                        }
                    }
                    ...
                    else if (quantityOptions.length === 0 && (data.data.size.trim() || data.data.color.trim())) {
                    ...
                    result = {
                        type: Types.ASK_PRODUCT, data: { product, quantityOptions: _quantityOptions, infoMore }
                    };
                }
                else {
            ...
            case Types.ASK_BEST_RATE:
                ...
                result = {
                    type: Types.ASK_BEST_RATE, data: product
                }
                break;
            case Types.ASK_BEST_SELL:
                ...
                result = {
                    type: Types.ASK_BEST_SELL, data: product
                }
                break;
            case Types.ASK_BEST_VIEW:
                ...
                result = {
                    type: Types.ASK_BEST_VIEW, data: product
                };
                break;
            case Types.ASK_BRANDS:
                const brands = await Brand.find({ status: true }).select('_id name').select('_id name').limit(8);
                result = { type: Types.ASK_BRANDS, data: brands };
                break;
            case Types.ASK_CATEGORIES:
                const categoryGroups = await CategoryGroup.find({ status: true }).select('_id name').limit(8);
                result = { type: Types.ASK_CATEGORIES, data: categoryGroups };
                break;
            case Types.ASK_CATEGORIES_T_SHIRT:
                categorys = await Category.find({ categoryGroupId: Types.T_SHIRT_CATEGORY_ID }).select('_id name').limit(8);
                result = { type: Types.ASK_CATEGORIES, data: categorys };
                break;
            case Types.ASK_CATEGORIES_SHIRT:
                categorys = await Category.find({ categoryGroupId: Types.SHIRT_CATEGORY_ID }).select('_id name').limit(8);
                result = { type: Types.ASK_CATEGORIES, data: categorys };
                break;
            case Types.ASK_CATEGORIES_JEAN:
                categorys = await Category.find({ categoryGroupId: Types.JEAN_CATEGORY_ID }).select('_id name').limit(8);
                result = { type: Types.ASK_CATEGORIES, data: categorys };
                break;
            case Types.ASK_CATEGORIES_JACKET:
                categorys = await Category.find({ categoryGroupId: Types.JACKET_CATEGORY_ID }).select('_id name').limit(8);
                result = { type: Types.ASK_CATEGORIES, data: categorys };
                break;
            ...
            case Types.ASK_PERSON_BOT_CARE:
                ...
                result = { type: 'msg', data: `Người tôi quan tâm nhất là ${user.name} vì anh ta tôi sẵn sàng làm mọi thứ!` };
                break;
...
module.exports = { getMessengers, updateMessengerCheck, sendMessageToUser, sendMessageToBot, getMessages }
