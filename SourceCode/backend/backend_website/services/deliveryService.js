...
module.exports.GHNcreateOrder = async function GHNcreateOrder(name, phone,address, quantity, serviceTypeId, serviceId, price) {
    try {
        ...
        return {shipOrderId: response.data.data.order_code, shippingFee:response.data.data.total_fee,
            ...
            for (var i = services.length - 1; i > 0; i--) {
                ...
                if (fee !== null && date !== null)
                    result.push({
                        ...
                        name: 'Giao hàng nhanh'+(services[i].serviceTypeId===1?' - Hỏa tốc':services[i].serviceTypeId===2?' - Giao nhanh':''),
                        shippingFee: fee,
                        date: date
                    });
            }
...
async function GHNgetAddressCode(address) {
    ...
    const proBestMatchResult = stringSimilarity.findBestMatch(address.city, provincesName);
    ...
    const disBestMatchResult = stringSimilarity.findBestMatch(address.district, districtsName);
    ...
    const wardBestMatchResult = stringSimilarity.findBestMatch(address.ward, wardsName);
