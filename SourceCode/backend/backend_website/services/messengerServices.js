...
module.exports.updateMessengerCheck = async (io, socketId, messengerId) => {
    try {
        ...
        const senderId = userOnline[index].userId != result.user1 ? result.user1 : result.user2;
        const reciverId = senderId != result.user1 ? result.user1 : result.user2;

        //1 trong 2 là null
        if (!senderId || !reciverId)
            for (let i = userOnline.length - 1; i >= 0; i--) {
                if (userOnline[i].role !== ROLE.CUSTOMER && userOnline[index].userId !== userOnline[i].userId)
...
module.exports.sendMessage = async (io, socketId, to, content, isCustomerCare) => {
    try {
        const index = Extention.findIndexById(userOnline, socketId);
        if (index === -1) {
            io.sockets.in(socketId).emit("verifyFail", {
        ...
        const result = await Message.sendMessageToUser(isCustomerCare ? null : userOnline[index].userId, to, content);
...
module.exports.sendMessageToBot = async (io, socketId, content) => {
    try {
        ...
        const index = await this.sendMessage(io, socketId, process.env.BOT_ID, content);
        if (index === -1)
            return;

        const result = await Message.sendMessageToBot(userOnline[index].userId, content);
    ...
    try {
        if (Extention.findIndexById(userOnline, socketId) >= 0) {
            ...
            io.sockets.in(socketId).emit("login", {
                socketId, userId: result.decoded.data._id
            });
            return true;
        }
    } catch (error) {
