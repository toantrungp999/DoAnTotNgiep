...
router.get('/districts/:id', async (req, res) => {
    const result = all.filter(city => city.id == req.params.id).map(city => city.districts);
    res.send(new Respones(0, result[0]));
});

router.get('/geocode', async (req, res) => {
    var data = req.query;
    const { error } = postAddressValidation({city: data.city, district:data.district,ward: data.ward, streetOrBuilding: data.streetOrBuilding,isDefault:true});
        if (error) return res.send(new Respones(STATUS.INPUT_DATA, null, error.details[0].message));
    try {
        const address = data.streetOrBuilding +','+data.ward + ','+data.district+','+data.city;
