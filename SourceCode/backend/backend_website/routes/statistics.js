    ...
    try {
        ...
        if (statisticsType === 'revenue') {
            ...
            for (var i = 0; i < orders.length;) {
                if (Date.parse(orders[i].date.toISOString()) >= Date.parse(startDate.toISOString()) + (j * 86400000) &&
                    Date.parse(orders[i].date.toISOString()) < Date.parse(startDate.toISOString()) + ((j + 1) * 86400000)
                ) {
                    sum += orders[i].totalPrice;
                    i++;
                }
                else {
                    revenue.push({ date: new Date(Date.parse(startDate.toISOString()) + (j * 86400000)), sum: sum });
                    sum = 0;
                    j++;
                }
                if (i == orders.length) {
            ...
            let orderDetails = await OrderDetail.find({ $or: orderIds })
                .populate({
                    path: 'quantityId', populate: { path: 'productId', select: ['name'] }
                });
            for (var i = 0; i < orderDetails.length; i++) {
                ...
                if (sales[name]){
                    ...
                    sales[name] = {quantity:orderDetails[i].quantity,total:orderDetails[i].price*orderDetails[i].quantity}
                }
            }
        }
