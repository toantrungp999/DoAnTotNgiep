...
const { postRateValidation, postReplyValidation, putRateValidation,
    putReplyValidation, deleteRateValidation, deleteReplyValidation }
...
router.get('/:productId&pagesize=:pagesize', async (req, res) => {
    ...
    try {
        const rates = await Rate.find({ productId: req.params.productId })
            ...
            .sort([['_id', -1]])
            .limit(pageSize);
        const total = await Rate.countDocuments({ productId: req.params.productId });
...
router.post('/', verify, async (req, res) => {
    ...
    try {
        const check = await Rate.findOne({ productId: req.body.productId, user: req.user._id, });
        if (check)
...
router.post('/replies', verify, async (req, res) => {
    ...
    if (error) return res.send(new Respones(STATUS.INPUT_DATA, null, error.details[0].message));
    try {
        await Rate.updateOne({ _id: req.body.rateId }, {
            "$push": {
                "replies": {
        ...
        const notification = new Notification({
            user: userId,
            target: { product: rate.productId, rate: rate._id, replyRate: rate.replies[rate.replies.length - 1]._id },
            performedBy: req.user._id,
            action: REPLIED_RATE
        });
        await notification.save();
...
router.put('/', verify, async (req, res) => {
    ...
    if (error) return res.send(new Respones(STATUS.INPUT_DATA, null, error.details[0].message));
    try {
        await Rate.updateOne({ _id: req.body.rateId, user: req.user._id }, {
            $set: {
                rate: req.body.rate,
                content: req.body.content
            }
...
router.put('/replies', verify, async (req, res) => {
    ...
    if (error) return res.send(new Respones(STATUS.INPUT_DATA, null, error.details[0].message));
    try {
        await Rate.updateOne({ _id: req.body.rateId, replies: { $elemMatch: { _id: req.body.replyId, user: req.user._id } } }, {
            $set: {
