...
router.get('/', async (req, res) => {
    ...
    return res.send(new Respones(result.codeStatus, { accessToken: result.accessToken }, result.message));
});
...
async function checkToken(refreshToken) {
    ...
    if (refreshToken && TOKEN_LIST && TOKEN_LIST[refreshToken])
        try {
            ...
            accessToken = await jwtHelper.generateToken(decoded.data, accessTokenSecret, accessTokenLife);
        } catch (error) {
