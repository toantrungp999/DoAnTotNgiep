...
const { postQuantityValidation, putQuantityValidation } = require('../validation/quantityValidation');
...
router.get('/productId=:productId', async (req, res) => {
    try {
        let quantityOptions = [];
        quantityOptions = await QuantityOption.find({ productId: req.params.productId });
...
router.get('/admin/productId=:productId', async (req, res) => {
    try {
        ...
        let quantityOptions = [];
        _quantityOptions = await QuantityOption.find({ productId: req.params.productId });
        for (var i = 0; i< _quantityOptions.length; i++) {
            let orderDetails = await OrderDetail.find({ quantityId: _quantityOptions[i]._id })
                .populate({ path: 'orderId', select: ['status'] })
              
            orderDetails = orderDetails.filter(orderDetail => orderDetail.orderId.status === OrderStatus.PENDING_APPROVE
