...
const { postCartValidation, putCartValidation, putUpdateTypeCartValidation } = require('../validation/cartValidation');
...
router.get('/', verify, async (req, res) => {
    try {
        ...
        for (let i = carts.length - 1; i >= 0; i--) {
            let quantityOption = await QuantityOption.findOne({ colorId: carts[i].colorId._id, sizeId: carts[i].sizeId._id });
            if (!quantityOption)
...
router.post('/', verify, async (req, res) => {
    ...
    try {
        const quantityOption = await QuantityOption.findOne({ colorId: req.body.colorId, sizeId: req.body.sizeId });
        if (!quantityOption)
        ...
        const checkCart = await Cart.findOne({ user: req.user._id, colorId: req.body.colorId, sizeId: req.body.sizeId })
            .populate({
                path: 'colorId', select: ['color', 'image', 'productId']
            })
            .populate({
        ...
        if (!checkCart) {
            if (total < quantity)
                return res.send(new Respones(-2, null, "Không đủ loại sản phẩm này trong kho!"));
            if (quantity > 5)
                return res.send(new Respones(-3, null, "Không thể thêm quá 5 sản phẩm cùng 1 loại vào giỏ hàng!"));
            const cart = new Cart({
            ...
            if (quantity > total)
                return res.send(new Respones(-2, null, "Đã tồn tại " + checkCart.quantity + " sản phẩm này. Không đủ loại sản phẩm này trong kho!"));
            if (quantity > 5)
                return res.send(new Respones(-3, null, "Không thể thêm quá 5 sản phẩm cùng 1 loại vào giỏ hàng!"));
            await Cart.updateOne({ _id: checkCart._id }, {
                $set: {
                    quantity
                }
            });
...
router.put('/', verify, async (req, res) => {
    ...
    try {
        ...
        if (checkCart && checkCart.user == req.user._id) {
            ...
            const quantityOption = await QuantityOption.findOne({ colorId: checkCart.colorId, sizeId: checkCart.sizeId });
            if (!quantityOption)
                return res.send(new Respones(-1, null, "Loại sản phẩm này tạm hết hàng"));
            let total = quantityOption.quantity;
            if (total === 0)
            ...
            if (quantity > 5)
                return res.send(new Respones(-3, null, "Không thể thêm quá 5 sản phẩm cùng 1 loại vào giỏ hàng!"));
            await Cart.updateOne({ _id: req.body._id }, {
                $set: {
                    quantity,
                    date: Date.now()
                }
            ...
            res.send(new Respones(-1, null, "Không tìm thấy giỏ hàng!"));
    } catch (error) {
        console.log(error);
        res.send(new Respones(STATUS.UNKNOWN_EEROR, null, error.message));
    }
...
router.put('/update-type', verify, async (req, res) => {
    ...
    try {
        let quantity = parseInt(req.body.quantity);
        const existCart = await Cart.findOne({ user: req.user._id, colorId: req.body.colorId, sizeId: req.body.sizeId });
        ...
        if (!sizeOption)
            return res.send(new Respones(-1, null, "SizeOption not found"));
        const quantityOptions = await QuantityOption.find({ colorId: req.body.colorId, sizeId: req.body.sizeId });
        if (!quantityOptions)
            return res.send(new Respones(-1, null, "QuantityOption not found"));
        let total = 0;
        ...
        if (existCart && existCart._id != req.body.cartId) {
            ...
            await Cart.updateOne({ _id: existCart._id }, {
                $set: {
        ...
        let quantityOption = await QuantityOption.findOne({ colorId: result.colorId._id, sizeId: result.sizeId._id });
        if (!quantityOption)
            return res.send(new Respones(-1, null, "QuantityOption not found"));
        let quantityInStore = quantityOption.quantity;
...
router.delete('/:_id', verify, async (req, res) => {
    try {
        const check = await Cart.deleteOne({ _id: req.params._id, user: req.user._id });
        if (check.n && check.n == 1)
            return res.send(new Respones(0, null));
        else
            res.send(new Respones(-1, null, "Không tìm thấy giỏ hàng!"));
    } catch (error) {
        res.send(new Respones(STATUS.UNKNOWN_EEROR, null, error.message));
