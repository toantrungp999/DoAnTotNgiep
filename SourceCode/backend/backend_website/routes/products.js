...
const { postProductValidation, putProductValidation } = require('../validation/productValidation');
...
router.get('/homepage/', async (req, res) => {
    try {
        ...
        const hots = await Product.find({ status: true })
            .select('_id name brandId categoryId images orgin material description price saleOff numberComment numberRate numberVisit avgRate status')
            .sort([['numberVisit', -1]])
            .limit(8);
        ...
        res.send(new Respones(0, { hots, bestSellers, news, colorOptions, sizeOptions }));
...
router.get('/recommended/userId=:userId&productId=:productId', async (req, res) => {
    try {
        ...
        if (!user || !user.visitProduct || !user.visitCategory)
            return res.send(new Respones(0, { recommendedProducts: [] }));
        var visitProduct = user.visitProduct.length>10?user.visitProduct.slice(0,9):user.visitProduct;
        var visitCategory = user.visitCategory.length>15?user.visitCategory.slice(0,14):user.visitCategory;
        let products = [];
        ...
        for (var i = 0; i < 2;) {
            ...
            if (mostProduct) {
                const product = await Product.findOne({ $and: condition.concat([{ _id: mostProduct }]) });
                if (product) {
                ...
                visitProduct = visitProduct.filter(product => product != mostProduct);
            } else {
                break;
            }
        }
        ...
        for (var i = 0; i < 6;) {
            ...
            if (mostCategory) {
                const product = await Product.find({ $and: condition.concat([{ categoryId: mostCategory }]) })
                .sort([['numberBuy', -1]])
                .limit(4);
                if (product) {
                ...
                visitCategory = visitCategory.filter(category => category != mostCategory);
            } else {
                break;
            }
        }
        ...
        if (user && productId && productId !== 'undefined') {
            ...
            if (product) {
                ...
                if (visitCategory) {
                    ...
                    visitCategory = [product.categoryId];
                // console.log(visitProduct);
                // console.log(visitCategory);
                await User.updateOne({ _id: userId }, { visitProduct: visitProduct, visitCategory: visitCategory });
            }
        }
        res.send(new Respones(0, { recommendedProducts: products, colorOptions, sizeOptions }));
...
router.get('/path=:path&key=:key&min=:min&max=:max&option=:option&page=:page&status=:status', async (req, res) => {
    const pageSize = 8;
    ...
    try {
        switch (path) {
            ...
            case 'category':
                ...
                const categoryName = await Category.findById(key).select('name categoryGroupId').populate('categoryGroupId', 'name');
                searchInfo = { categoryName }
                break;
            case 'category-group':
                const categorys = await Category.find({ categoryGroupId: key, status: true });
                const categoryGroupName = await CategoryGroup.findById(key).select('name');
            ...
            case 'all':
                break;
            case 'black-color':
                const colors = await ColorOption.find({ color: 'Đen' }).select('productId');
                condition = [];
        ...
        condition.$and = [{ price: { $gte: Number(min) } }, { price: { $lte: Number(max) } }];
        if (sort.length === 0) {
        ...
        res.send(new Respones(0, { products, colorOptions, sizeOptions, pagingInfo: { totalPage, currentPage, pageSize, option }, searchInfo }
        ));
        console.log(Date.now());
    } catch (error) {
        console.log(error.message)
...
router.get('/admin', async (req, res) => {
    ...
    try {
        ...
        res.send(new Respones(0, { products, pagingInfo: { totalPage, currentPage, pageSize } }));
    } catch (error) {
        res.send(new Respones(STATUS.UNKNOWN_EEROR, null, error.message));
    }
});
...
//option 3 is bestseller
//option 4 is most prominent
router.get('/searchString=:searchString&page=:page&pagesize=:pagesize&option=:option&status=:status', async (req, res) => {
    ...
    try {
        ...
        const categories = await Category.find(
            { $or: [{ _id: { $in: categoryGroups }, name: { $regex: searchString } }] },
            { _id: 1 });
        const condition = { $or: [{ brandId: { $in: brands } }, { categoryId: { $in: categories } }, { name: { $regex: searchString } }, { status: status }] };
        const products = await Product.find(condition)
            .select('_id name brandId categoryId images orgin material description price numberComment numberRate numberVisit avgRate status')
            .populate('brandId', 'name')
            .populate('categoryId', 'name')
            .sort([[sort, direction]])
            .skip(pageSize * (currentPage - 1))
        ...
        if ((totalPage - 1) * pageSize >= count)
            totalPage--;
        res.send(new Respones(0, { products, pagingInfo: { totalPage, currentPage } }));
    } catch (error) {
        res.send(new Respones(STATUS.UNKNOWN_EEROR, null, error.message));
    }
});
...
router.post('/', verifyTokenSuperAdmin, async (req, res) => {
    ...
    try {
        ...
        if (!brand)
            return res.send(new Respones(-1, null, "Không tìm thấy hãng"));
...
router.get('/:id', async (req, res) => {
    try {
        ...
        await Product.updateOne({ _id: req.params.id }, { $inc: { numberVisit: 1 } });
...
router.get('/admin/:_id', async (req, res) => {
    try {
        ...
        if (!product)
            return res.send(new Respones(-1, null, "Không tìm thấy sản phẩm"));
        await Product.updateOne({ _id: req.params._id }, { $inc: { numberVisit: 1 } });
