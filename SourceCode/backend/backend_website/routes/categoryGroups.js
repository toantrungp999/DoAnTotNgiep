...
const { postCategoryGroupValidation, putCategoryGroupValidation } = require('../validation/categoryGroupValidation');
...
router.get('/:status', async (req, res) => {
    try {
        ...
        if (status == 'all')
            ...
            status == false;
        const categoryGroups = await CategoryGroup.find(status ? { status } : {}).sort([['date','-1']]);
...
router.get('/with-category/:status', async (req, res) => {
    try {
        ...
        for (var i = 0; i < _categoryGroups.length; i++) {
            const categorys = await Category.find({ categoryGroupId: _categoryGroups[i]._id});
            categoryGroups.push({ categoryGroup:  _categoryGroups[i], categorys: categorys })
        }
...
router.put('/', verifyTokenSuperAdmin, async (req, res) => {
    ...
    try {
        ...
        await Category.updateMany({ categoryGroupId: req.body._id }, { $set: { status: req.body.status } });
        const categorys = await Category.find({ categoryGroupId: req.body._id });
        for(var i=0;i<categorys.length;i++){
            await Product.updateMany({ categoryId: categorys[i]._id}, { $set: { status: req.body.status } });
        }
