...
const { approveOrderValidation, updateSerialValidation } = require('../validation/orderValidation');
...
const { GHNshippingFee, GHNcreateOrder } = require('../services/deliveryService');
const OrderTypes = require('../constants/OrderType');
const storeAddresses = require('../resources/storeLocation.json');
const sendMail = require('./../services/SendMail');
const Notification = require('../models/Notification');
const { ORDERED, APPROVAL, CANCELED_ORDER } = require("../constants/NotificationAction");
...
router.get('', verify, async (req, res) => {
    try {
        ...
        const orderDetails = await OrderDetail.find({ $and: [{ $or: conditions }, { name: { $regex: search, $options: 'i' } }] });
        ...
        conditions.push({ "customerInfo.name": { $regex: search, $options: 'i' } },
            { "customerInfo.phoneNumber": { $regex: search, $options: 'i' } },
            { orderId: { $regex: search, $options: 'i' } });
        if (status == 'Tất cả')
            conditions = { $and: [{ $or: conditions }, { userId: req.user._id }] };
        else
            conditions = { $and: [{ $or: conditions }, { userId: req.user._id }, { status: status }] };
        ...
        let orders = [];
        for (const i in orderInfos) {
            const orderDetails = await OrderDetail.find({ orderId: orderInfos[i]._id });
            orders.push({
...
router.get('/all', verifyTokenAdmin, async (req, res) => {
    try {
        ...
        if (status == 'Tất cả')
            ...
            orderInfos = await Order.find({ status: status });
        ...
        if (status == 'Tất cả')
            ...
            conditions = { $and: [{ $or: conditions }, { status: status }] };
...
router.get('/:_id', verify, async (req, res) => {
    try {
        ...
        if (orderInfo.userId != req.user._id)
            return res.send(new Respones(STATUS.ERROR_PERMISSIONS, null, "Không có quyền truy cập"));
        const orderDetails = await OrderDetail.find({ orderId: orderInfo._id });
        let paymentInfoUrl = null;
        if (orderInfo.paymentType === OrderTypes.ONLINE_ORDER.PAYMENT_TYPE.VNPAY
        ...
        res.send(new Respones(0, {
            orderInfo, orderDetails, paymentInfoUrl
        }));
    } catch (error) {
        console.log(error);
    ...
    try {
        ...
        for (let index = cartIdList.length - 1; index >= 0; index--) {
            const cart = await Cart.findById({ _id: cartIdList[index] })
                .populate({
                    path: 'colorId', select: ['color', 'image', 'productId']
                })
                .populate({
                    path: 'colorId', populate: { path: 'productId', select: ['name', 'price', 'saleOff'] }
                }).populate({
                    path: 'sizeId', select: ['size']
                });
            if (cart) {
                carts.push(cart);
            }
        ...
        if (carts.length > 0)
            res.send(new Respones(0, carts));
        else
            throw new Error('Không lấy được dữ liệu, chưa chọn sản phẩm')
    } catch (error) {
        res.send(new Respones(STATUS.UNKNOWN_EEROR, null, error.message));
    }
});
...
async function createOrder(req, res) {
    ...
    try {
        ...
        for (const index in options) {
            const quantityOption = await QuantityOption.findOne({ colorId: options[index].colorId, sizeId: options[index].sizeId })
                .populate({ path: 'productId', select: ['name', 'brandID', 'price', 'saleOff'] })
            ...
            if (quantityOption.quantity < options[index].quantity)
                throw new Error(`${quantityOption.productId.name} số lượng còn lại không đủ, vui lòng thử lại sau`);
            //console.log("priceOptionId", options[index]._id);
            //console.log("priceOption", priceOption);
            const brand = await Brand.findById(quantityOption.productId.brandId).session(session);
            const price = quantityOption.productId.price - quantityOption.productId.saleOff;
            ...
            await Cart.deleteOne({ user: req.user._id, colorId: options[index].colorId, sizeId: options[index].sizeId }).session(session);
        ...
        if (orderType == OrderTypes.ONLINE_ORDER.ORDER_TYPE) {
            ...
            if (receiveType == OrderTypes.ONLINE_ORDER.RECEIVE_TYPE.SHIP) {
                ...
                shipOrderInfo = await GHNcreateOrder(userInfo.name, userInfo.phoneNumber, address, totalQuantity, shipInfo.serviceTypeId,
                    shipInfo.serviceId, totalPrice);
                order.receiveAddress = address.streetOrBuilding + ', ' + address.ward
                ...
                if (shipOrderInfo !== null) {
                    ...
                    throw new Error('Liên kết với đơn vị giao hàng thất bại - Vui lòng sử dụng số điện thoại thật để đảm bảo tính chính xác');
                }
            }
        ...
        sendMail("Đặt hàng thành công", user.email, "<span>Bạn đã đặt hành thành công, đơn hàng của bạn là " + order.orderId + " </span>");
...
router.put('/admin/:_id', verifyTokenAdmin, async (req, res) => {
    ...
    if (type === OrderActions.CANCEL || type === OrderActions.PASS || type === OrderActions.FAIL || type === OrderActions.APPROVE) {
        changeType(req, res);
    }
    else {
        return res.send(new Respones(STATUS.UNKNOWN_EEROR, null, 'Thao tác thất bại'));
    }
});
...
async function changeType(req, res) {
    ...
    try {
        ...
        else if (description == undefined || description == null || description.length < 0 || description.length > 500) {
        ...
        if (type == OrderActions.PAY) {
            ...
            if (order.status == OrderStatus.PENDING_PAY && order.paymentType == OrderTypes.ONLINE_ORDER.PAYMENT_TYPE.ONLINE) {
                await Order.updateOne({ _id: req.params._id }, {
                    $set: {
                        status: OrderStatus.PENDING_APPROVE
                    }
                ...
                emailContent = "Đơn hàng " + order.orderId + " của bạn đã được thanh toán thành công";
            ...
            if (order.status == OrderStatus.PENDING_APPROVE) {
                const orderDetails = await OrderDetail.find({ orderId: order._id }).session(session);
                for (let index in orderDetails) {
                    let quantityOption = await QuantityOption.findById(orderDetails[index].quantityId)
                        .populate('productId', 'name')
                        .session(session);
                    if (quantityOption.quantity >= orderDetails[index].quantity) {
                        quantityOption.quantity -= orderDetails[index].quantity;
                        await quantityOption.save();
                        let product = await Product.findById(quantityOption.productId).session(session);
                        product.numberBuy += orderDetails[index].quantity;
                        await product.save();
                    } else
                        throw new Error(`Duyệt đơn hàng thất bại, số lượng không đủ - ${quantityOption.productId.name}`);//đã kt ở giao diện, nên chỉ thông báo
                }
                await Order.updateOne({ _id: req.params._id }, {
                    $set: {
                        status: OrderStatus.DELIVERING
                ...
                await notification.save();
                sendMail(emailTitle, order.userId.email, emailContent);
                return res.send(new Respones(0, { status: OrderStatus.DELIVERING, actionLog: order.actionLog }));
                // return res.json({
                //     message: 'Duyệt đơn hàng thành công',
                //     status: OrderStatus.DELIVERING
                // });
            ...
            if (order.status == OrderStatus.DELIVERING) {
                ...
                emailContent = "Đơn hàng " + order.orderId + " của bạn đã được giao thành công"
                return res.send(new Respones(0, { status: OrderStatus.COMPLETED, actionLog: order.actionLog }, 'Đã xác nhận giao hàng thành công'));
            } else {
            ...
            if (order.status == OrderStatus.DELIVERING) {
                ...
                emailContent = "Đơn hàng " + order.orderId + " của bạn đã đã bị hủy do giao hàng không thành công"
                return res.send(new Respones(0, { status: OrderStatus.FAILED, actionLog: order.actionLog }, 'Đã xác nhận giao hàng thất bại'));
            } else {
                throw new Error("Thao tác xác nhận thất bại");
            }
        }
        else if (type === OrderActions.CANCEL) {
            if (order.status == OrderStatus.PENDING_PAY || order.status == OrderStatus.PENDING_APPROVE) {
                await Order.updateOne({ _id: req.params._id }, {
                    $set: {
                ...
                await notification.save();
                return res.send(new Respones(0, { status: OrderStatus.CANCELED, actionLog: order.actionLog }, 'Hủy đơn hàng thành công'));
            } else {
...
router.get('/payment/vnpay_url/:id', verify, async (req, res) => {
    try {
        ...
        if (order.paymentType !== OrderTypes.ONLINE_ORDER.PAYMENT_TYPE.VNPAY || order.status !== OrderStatus.PENDING_PAY)
            return res.send(new Respones(0, {}));
...
router.get('/payment/vnpay_ipn', async (req, res) => {
    try {
        ...
        if (secureHash === checkSum) {
            ...
            if (rspCode === '00') {
                var order = await Order.findOne({ paymentId: paymentId })
                ...
                order.status = OrderStatus.PENDING_APPROVE;
                sendMail('Thanh toán thành công', order.userId.email, `<span>Đơn hàng ${order.orderId} của bạn đã được thanh toán thành công trên cổng dịch vụ VNPAY</span>`);
                await order.save();
