...
const { postCommentValidation, postReplyValidation, putCommentValidation,
    putReplyValidation, deleteCommentValidation, deleteReplyValidation }
    = require('../validation/commentValidation');
const { COMMENTED, REPLIED_COMMENT } = require("../constants/NotificationAction");
...
router.get('/:productId&pagesize=:pagesize', async (req, res) => {
    ...
    try {
        const comments = await Comment.find({ productId: req.params.productId })
            ...
            .sort([['_id', -1]])
            .limit(pageSize);
        const total = await Comment.countDocuments({ productId: req.params.productId });
...
router.post('/replies', verify, async (req, res) => {
    ...
    try {
        ...
        const comment = await Comment.findById(req.body.commentId)
            ...
            .populate({ path: 'replies.user', select: ['name', 'image', 'role'] })
            .sort([['_id', -1]]);
        await Product.updateOne({ _id: comment.productId },
            { $inc: { numberComment: 1 } }
        );
        const userId = comment.user._id != req.user._id ? comment.user._id : null;
        const notification = new Notification({
            user: userId,
            target: { product: comment.productId, comment: comment._id, replyComment: comment.replies[comment.replies.length - 1]._id },
            performedBy: req.user._id,
...
router.put('/', verify, async (req, res) => {
    ...
    if (error) return res.send(new Respones(STATUS.INPUT_DATA, null, error.details[0].message));
    try {
        await Comment.updateOne({ _id: req.body.commentId, user: req.user._id }, {
            $set: {
                content: req.body.content
            }
        });
...
router.put('/replies', verify, async (req, res) => {
    ...
    if (error) return res.send(new Respones(STATUS.INPUT_DATA, null, error.details[0].message));
    try {
        await Comment.updateOne({ _id: req.body.commentId, replies: { $elemMatch: { _id: req.body.replyId, user: req.user._id } } }, {
            $set: {
