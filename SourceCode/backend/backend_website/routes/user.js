...
const { updateAvatarValidation, putProfileValidation,
    putPasswordValidation, postAddressValidation, putAddressValidation, putPhoneValidation } = require('./../validation/userValidation')
    ...
    try {
        ...
        await User.updateOne(
            { _id },
            {
                $set: {
                    name: req.body.name,
                    birthday: req.body.birthday, male: req.body.male
                }
            });
        const _user = await User.findById(_id);
        const user = {
...
router.put('/password', verify, async (req, res) => {
    ...
    try {
        ...
        if (vaildPass) {
            ...
            res.send(new Respones(-1, null, "Sai mật khẩu"));
    } catch (error) {
        res.send(new Respones(STATUS.UNKNOWN_EEROR, null, error.message));
    }
});
...
router.put('/addresses', verify, async (req, res) => {
    ...
    try {
        ...
        if (req.body.isDefault) {
            ...
            await User.updateOne({ _id, addresses: { $elemMatch: { _id: req.body._id } } }, {
                $set: {
...
router.delete('/addresses/:addressId', verify, async (req, res) => {
    try {
        await User.updateOne({ _id: req.user._id, "addresses._id": req.params.addressId }, {
            $set: {
