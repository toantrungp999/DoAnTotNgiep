...
const { postCategoryValidation, putCategoryValidation } = require('../validation/categoryValidation');
...
router.get('/:status', async (req, res) => {
    try {
        ...
        if (status == 'all')
            ...
            status == false;
        const categories = await Category.find(status ? { status } : {}).sort([['date','-1']]);
...
router.put('/', verifyTokenSuperAdmin, async (req, res) => {
    ...
    try {
        ...
        await Category.updateOne(
            { _id: req.body._id },
            { $set: { name: req.body.name, categoryGroupId: req.body.categoryGroupId, status: req.body.status } });
        await Product.updateMany({ categoryId: req.body._id }, { $set: { status: req.body.status } });
