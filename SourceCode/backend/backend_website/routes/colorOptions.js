...
const { postColorValidation, putColorValidation } = require('../validation/colorValidation');
...
router.post('/', verifyTokenSuperAdmin, async (req, res) => {
    ...
    try {
        ...
        if (!product)
            res.send(new Respones(-1, null, "Không tìm thấy sản phẩm"));
...
router.get('/product/:productId', async (req, res) => {
    try {
        ...
        if (!product)
            res.send(new Respones(-1, null, "Không tìm thấy sản phẩm"));
        const colorOptions = await ColorOption.find({ productId: req.params.productId });
