...
const { postBrandValidation, putBrandValidation } = require('../validation/brandValidation');
...
router.put('/', verifyTokenSuperAdmin, async (req, res) => {
    ...
    try {
        await Brand.updateOne(
            { _id: req.body._id },
            { $set: { name: req.body.name, description: req.body.description ? req.body.description : '', status: req.body.status } });
        await Product.updateMany({ brandId: req.body._id }, { $set: { status: req.body.status } });
