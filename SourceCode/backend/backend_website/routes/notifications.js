    ...
    try {
        ...
        if (_notifications !== null)
            for (let i = _notifications.length - 1; i >= 0; i--)
                if (_notifications[i].check === false)
                    notSeen++;
        const notifications = await Notification.find(condition).populate({ path: 'performedBy', select: ['name', 'image', 'role'] }).populate({ path: 'target.product', select: ['name'] })
            .sort([['_id', -1]])
            .limit(pageSize);
...
router.get('/mobile/pageSize=:pageSize&page=:page', verify, async (req, res) => {
    try {
        const pageSize = parseInt(req.params.pageSize);
        ...
        if ((totalPage - 1) * pageSize >= count)
            totalPage--;
        res.send(new Respones(0, { notifications, pagingInfo: { totalPage, currentPage, pageSize, notSeen } }));
    } catch (error) {
        res.send(new Respones(STATUS.UNKNOWN_EEROR, null, error.message));
    }
});
