...
class CreateOrderPage extends Component {
    ...
    render() {
        let { loading, carts, createSuccess, orderInfo } = this.props.createOrderReducer;
        if (loading)
                    ...
                    <div className={'product-item '+(index===0&&'first')}>
                        ...
                        <div className='product-info'>
                            ...
                            <span className='price-quantity'>
                                {saleOff !== 0 ? <del className='orgin-price'>{convertNumberToVND(price)}₫</del> : ''}
                                <span className='price'>{convertNumberToVND(price - saleOff)}₫</span>
                                <span className='quantity'>x{quantity}</span>
                            </span>
                            <span className='total-price price'>{convertNumberToVND(price * quantity - saleOff * quantity)}₫</span>
                        </div>
                    </div>
            )
        }) : '';
        ...
        return (
            <div className="create-order-page">
                <Link to={'/cart'} className='btn-back-create-order'><i className="fas fa-chevron-left pr-2"></i>TRỞ LẠI</Link>
...
export default connect(mapStateToProps, mapDispatchToProps)(memo(CreateOrderPage));