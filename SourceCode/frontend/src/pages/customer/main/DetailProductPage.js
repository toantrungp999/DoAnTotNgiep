...
class DetailProductPage extends Component {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (nextProps) {
      let { productDetailReducer, productOptionsReducer, cartsReducer } = nextProps;
      if (!productDetailReducer.colorLoading && !productDetailReducer.sizeLoading && !productDetailReducer.quantityLoading) {
        ...
        this.setState({ images: listImage, startIndex: 0, endIndex: listImage.length >= 5 ? 5 : listImage.length });
      }
  ...
  componentDidUpdate(prevProps) {
    ...
    if (!loading && this.props.location !== prevProps.location) {
      ...
      if (_id) {
        ...
        this.props.fectchProduct(_id, this.props.userInfoReducer.userInfo ? this.props.userInfoReducer.userInfo._id : undefined);
        this.props.fectchColorOptions(_id);
        this.props.fectchSizeOptions(_id);
        this.props.fectchQuantityOptions(_id);
        this.props.fectchComments(_id, this.state.lengthCmt);
    ...
    if (!this.props.userInfoReducer.userInfo && prevProps.userInfoReducer.userInfo ) {
      let _id = this.props.match.params._id;
      this.props.fectchRecommendedProducts(this.props.userInfoReducer.userInfo ? this.props.userInfoReducer.userInfo._id : undefined, _id);
    }
  ...
  onZoomImage = (e) => {
    this.setState({ zoomX: e.nativeEvent.offsetX, zoomY: e.nativeEvent.offsetY });
  }

  render() {
    const { recommendedProducts } = this.props.recommendedProductsReducer;
    const recommendedColorOptions = this.props.recommendedProductsReducer.colorOptions;
    const recommendedSizeOptions = this.props.recommendedProductsReducer.sizeOptions;
    const width = window.innerWidth;
    const style = width <= 768 ? { left: `${this.state.top}px` } : { top: `${this.state.top}px` };
    ...
    if (loading)
      ...
      const { name, price, orgin, saleOff, material, description, images, review, categoryId } = product;
      ...
      productOption = <ProductOption price={price} saleOff={saleOff} categoryId={categoryId} productOptionsReducer={this.props.productOptionsReducer}
        onShowColorImage={this.onShowColorImage} onAddCart={this.onAddCart} onBuy={this.onBuy} createLoading={createLoading} />
      const { userInfo } = this.props.userInfoReducer;
      const { currentShow } = this.state;
      const stateImages = this.state.images;
      const allImage = stateImages ? stateImages.map((image, index) => {
        return (<div key={index} onClick={() => this.onShow(index)} className={currentShow === index ? 'more-image choose-image' : 'more-image'}><img src={image} alt={123} /></div>)
      }) : '';
      ...
      const commentAndRate = <>
        <div className=''>
          ...
          <Rates viewMoreRates={this.viewMoreRates} lengthRate={this.state.lengthRate} totalRate={this.props.ratesReducer.total}
            ...
            ratesReducer={this.props.ratesReducer} userInfo={userInfo} />

        </div>
        <div className=''>

          <Comments viewMoreComments={this.viewMoreComments} lengthCmt={this.state.lengthCmt} totalCmt={this.props.commentsReducer.total}
            ...
            commentsReducer={this.props.commentsReducer} userInfo={userInfo} />
      ...
      const descriptionArea =
        <div>
          <div className="mt-20 pl-4 pr-5" dangerouslySetInnerHTML={{ __html: review ? htmlDecode(review) : '' }}></div>
        </div>;
          ...
          <table className="table text-left table-bordered table-information">
            <tbody>
              ...
              <tr>
                ...
                <td><p style={{ whiteSpace: 'pre-line', marginBlockStart: '0px' }}>{description}</p></td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      return (
        <main>
          ...
          <div className="product-detail-page">
            ...
            {this.state.showDialogSuccess ? <MessageSuccess message="Thêm sản phẩm vào giỏ hàng thành công" onOkay={this.onOkay} /> : null}
            {this.state.showDialogError ? <MeesageError message={messageCreate} onClose={this.onCloseDialog} /> : null}
            <div className="product-section">
              <div className="image-section">
                <div className="more-image-section">
                  {/* //this is container */}
                  <div>
                    <button onClick={this.onPrevious} className="btn-top-left"><UpOutlined /></button>
                    <div className='more-images-container'>
                      <div className='more-images' style={style} ref={this.moreImagesComponent}>
                        {allImage}
                      </div>
                    </div>
                    <button onClick={this.onNext} className="btn-bottom-right"><DownOutlined /></button>
                  </div>
                </div>
                <div className="selected-image-section" onMouseMove={this.onZoomImage}>
                  {/* //this is container */}
                  <div>
                    <ReactImageMagnify className='selected-image' {...{
                      smallImage: {
                        alt: 'Lỗi rồi nè',
                        isFluidWidth: true,
                        src: stateImages && stateImages.length > 0 ? stateImages[this.state.currentShow] : images[this.state.currentShow]
                      },
                      largeImage: {
                        src: stateImages && stateImages.length > 0 ? stateImages[this.state.currentShow] : images[this.state.currentShow],
                        width: 1000,
            ...
            <div className='view-type'>
              <div className='view-type-header'>
                <div className={'description view-type-title' + (viewType === 'description' ? ' active' : '')}>
                  <button onClick={(e) => this.changeViewType(e, 'description')}><span className='button-description'>Mô tả</span><span className='button-icon'></span></button>
                  <div className='view-type-content-inside'>
                    <Collapse in={viewType === 'description' ? true : false} timeout={500} collapsedHeight={0} >
                      {descriptionArea}
                    </Collapse>
                  </div>
                </div>
                <div className={'information view-type-title' + (viewType === 'information' ? ' active' : '')}>
                  <button onClick={(e) => this.changeViewType(e, 'information')}><span className='button-description'>Thông tin</span><span className='button-icon'></span></button>
                  <div className='view-type-content-inside'>
                    <Collapse in={viewType === 'information' ? true : false} timeout={500} collapsedHeight={0} >
                      {information}
                    </Collapse>
                  </div>
                </div>
                <div className={'review view-type-title' + (viewType === 'review' ? ' active' : '')}>
                  <button onClick={(e) => this.changeViewType(e, 'review')}><span className='button-description'>Đánh giá</span><span className='button-icon'></span></button>
                  <div className='view-type-content-inside'>
                    <Collapse in={viewType === 'review' ? true : false} timeout={500} collapsedHeight={0} >
                      {commentAndRate}
                    </Collapse>
                  </div>
                </div>
              ...
              <div className='view-type-content-outside'>
                {viewType === 'description' ? <div className='description'>{descriptionArea}</div> : ''}
                {viewType === 'information' ? <div className='information'>{information}</div> : ''}
                {viewType === 'review' ? <div className='review'>{commentAndRate}</div> : ''}
              </div>
            </div>
            {recommendedProducts && <RecommendedSection products={recommendedProducts} sizeOptions={recommendedSizeOptions} colorOptions={recommendedColorOptions} title='Gợi ý' description='Có thể bạn quan tâm' />}
          </div>
        </main>
      )
    }
...
const mapDispatchToProps = dispatch => ({
  ...
  fectchProduct: (productId, userId) => { dispatch(fectchProductRequest(productId, userId)) },
