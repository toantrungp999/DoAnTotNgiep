...
class OrderDetailPage extends Component {
    ...
    onConfirmCancel = (reason) => {
        ...
        this.props.orderChangeTypeRequest('customer', this.props.match.params._id, data);
        this.setState({
            showCancelDialog: false,
        });
        this.props.fetchOrderRequest(this.props.match.params._id, '');
    ...
    render() {
        ...
        if (this.props.orderDetailReducer.order === null) {
            return (<div className='row' style={{ height: '100%' }}><NotFound link={'/orders/'}></NotFound></div>)
        }
        ...
        return (
            <div className='order-detail-page'>
                ...
                <div className='ship-info'>
                    ...
                    <div className='receive-date'><InfoCircleOutlined /> Mã giao dịch: <label>{orderInfo.paymentId}</label></div>
                </div> : ''}
                <div className='order-detail'>
                    ...
                    <div>
                        {orderDetails.map((orderDetail, index) => {
                            return <OrderDetailItem
                                key={orderDetail._id} orderDetail={orderDetail} showInstore={false}
                            ></OrderDetailItem>
                        })}
                    </div>
                    <div className='bottom-section'>
                        ...
                        <div className="price-section">
                            ...
                            <div className=''>
                                <label className='total'>Tổng tiền: <label className='price'>{convertNumberToVND(orderInfo.total)}₫</label></label>
                            </div>
                        </div>
                    </div>
                    <div className='btn-section'>
                        ...
                        {orderInfo.status === OrderStatuses.PENDING_PAY ?
                            <div>
                                {payLoading ? <button className='btn-pay' name='pay' >Đang tải...</button> :
                                    <button className='btn-pay' name='pay' onClick={this.onChangeType}>Thanh toán</button>}
                            </div> : ''}
...
const mapDispatchToProps = dispatch => ({
    ...
    showAlertWithTimeout: (message, description, type) => { dispatch(showAlertWithTimeout(message, description, type)) }
})
