...
import {
  updateCartRequest, deleteCartRequest, fectchCartsRequest
...
class BasketCartPage extends Component {
  ...
  componentDidMount() {
    let cartId = this.props.location.state !== undefined ? this.props.location.state.cartId : null;
  ...
  render() {
    ...
    if (loading || this.state.cartId === undefined)
      ...
      let elementCarts = carts ? carts.map((cart, index) => {
        ...
        let { size } = sizeId;
        total += this.state.selectedList.includes(cart._id) ? productId.price * quantity - productId.saleOff * quantity : 0;
        totalSelectQuantity += this.state.selectedList.includes(cart._id) ? quantity : 0;
        totalQuantity += quantity;
        let type = colorId.color + ' - ' + size;
        let { cartId, selectedList } = this.state;
        let defaultCheck = cartId !== null ? cartId === cart._id ? true : false : true;
        let checked = selectedList.includes(cart._id);

        return <ItemBasketCart
          ...
          checked={checked} defaultCheck={defaultCheck} />
      }) : '';
                ...
                <div className='bottom-bar'>
                  <div className='fixed'>
                    <div className='check-section'>
                      <input id='checkbox-on-header' className='checkbox-cart-item' type="checkbox" name="isChoose" checked={this.state.selectAll} onChange={this.onCheck} />
                      <label className='label-checkbox-cart-item on-header' htmlFor='checkbox-on-header'><i className="fa fa-check item-checkbox-cart-item"></i></label>
                      <span className='pl-2'><strong>Chọn tất cả ({totalQuantity}) </strong></span>
                    </div>
                    <div className='buy-section'>
                      <div className='price-section'>
                        {this.state.selectedList.length === 0 ?
                          ...
                          :
                          <><span className='total-product'>Tổng ({totalSelectQuantity} Sản phẩm): </span>
                            <span className='price'>{convertNumberToVND(total)}₫</span></>}
                      </div>
                      <div className='btn-section'>
                        <button className="btn-buyy" onClick={this.onBuy}>Mua hàng</button>
                      </div>
                    </div>
                  </div>
                </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(memo(BasketCartPage));
