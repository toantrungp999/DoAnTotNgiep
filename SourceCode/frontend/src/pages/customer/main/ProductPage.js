...
class ProductPage extends React.Component {
    componentDidMount() {
        ...
        let { key, min, max, option, page } = queryString.parse(this.props.location.search);
    ...
    updateUrl = (newMin, newMax, newOption, newPage) => {
        ...
        let url = `/${path}?`;
        if (key) url += `key=${key}`;
        if (min !== undefined && min !== null && min.toString() !== '0') url += `&min=${min}`;
    ...
    render() {
        const { loading, products, pagingInfo, searchInfo, colorOptions, sizeOptions } = this.props.productsReducer;

        if (loading) {
            return (<div className='product-page'>
                <PageTitle location={this.props.location} searchInfo={searchInfo} onChangeSortOption={this.onChangeSortOption} />
        ...
        let productElements = products && products.length > 0 ? products.map((product, index) => {
            return <ProductItem key={product._id} index={index} product={product} colorOptions={colorOptions} sizeOptions={sizeOptions} />
        }) : <div className='no-data'>Không tìm thấy sản phẩm</div>;
        return (
            <div className='product-page'>
                <PageTitle location={this.props.location} searchInfo={searchInfo} onChangeSortOption={this.onChangeSortOption} onChangePriceRange={this.onChangePriceRange} />
                <div className='products'>
                    {productElements}
                </div>
