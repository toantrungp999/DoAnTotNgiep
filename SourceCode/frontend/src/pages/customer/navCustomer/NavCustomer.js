...
import {
    InfoOutlined, UserOutlined, EnvironmentOutlined, PhoneOutlined, KeyOutlined, FileDoneOutlined,
...
class NavCustomer extends Component {
    ...
    render() {
        ...
        const { image } = userProfileReducer.userProfile ? userProfileReducer.userProfile : '';
        ...
        return (
            <div className='nav-customer-page'>
                ...
                <div className={'nav-customer' + (this.state.showNav ? ' is-show' : '')}>
                    <div className={'show-hide' + (this.state.showNav ? ' is-show' : '')} onClick={this.showNav}>
                        {this.state.showNav ? <DoubleLeftOutlined /> : <DoubleRightOutlined />}
                    </div>
                    ...
                    <div className='account'>
                        <Link className={isChangeAccount ? 'active' : ''} to='/account/profile' onClick={this.showNav} ><UserOutlined className='icon account' />Tài khoản</Link>
                    </div>
                    <Collapse in={isChangeAccount} >
                        <div className='accounts'>
                            <div>
                                <Link className={isProfile ? 'active' : ''} to='/account/profile' onClick={this.showNav}><InfoOutlined className='icon info' />Thông tin</Link>
                            </div>
                            <div>
                                <Link className={isAddresses ? 'active' : ''} to='/account/addresses' name="showAddress" onClick={this.showNav}><EnvironmentOutlined className='icon address' />Địa chỉ</Link>
                            </div>
                            <div>
                                <Link className={isPassword ? 'active' : ''} to='/account/password' name="showChangepassword" onClick={this.showNav}><KeyOutlined className='icon password' />Mật khẩu</Link>
                            </div>
                            <div>
                                <Link className={isPhoneNumber ? 'active' : ''} to='/account/phonenumber' name="showChangePhone" onClick={this.showNav}><PhoneOutlined className='icon phone' />Số điện thoại</Link>
                            </div>
                        </div>
                    </Collapse>
                    <div className='order'>
                        <Link className={isOrders ? 'active' : ''} to='/orders' onClick={this.showNav} ><FileDoneOutlined className='icon order' />Đơn hàng</Link>
                    </div>
