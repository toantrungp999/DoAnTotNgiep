...
class ResetPassword extends Component {
    ...
    render() {
        var { message, loadingReset, statusReset } = this.props.userForgotPasswordReducer;
        var button_submit = loadingReset ? <input type="button" className="btn btn-primary btn-block" value="LOADING..." />
            : <input type="submit" className="btn btn-primary btn-block" value="Xác nhận" />
        var messageStatus = statusReset ? <div className="mt-10"><span className="text-success">Mật khẩu của bạn đã được cập nhật thành công.</span></div>
            : '';
        ...
        if (recheckPassword.length > 0 && password !== recheckPassword) {
            elemntTextRecheckPassword =
                <span className="text-danger mt-10">Nhập lại mật khẩu không chính xác</span>
        }
        return <div>
                ...
                <div className="row justify-content-center align-items-center">
                    <div className="col-xs-4 col-sm-4 col-md-4 col-lg-4">
                        ...
                        <div className="form-group row mt-20">
                            <input className="form-control" placeholder="Mật khẩu mới" name="password" onChange={this.onChange} type="password" minLength={6} required />
                        </div>
                        <div className="form-group row">
                            <input className="form-control" placeholder="Nhập lại mật khẩu" name="recheckPassword" onChange={this.onChange} type="password" minLength={6} required />
                        </div>
...
const mapDispatchToProps = dispatch => ({
    resetPassword: (code, password) => dispatch(resetPasswordRequest(code, password))
})

export default connect(mapStateToProps, mapDispatchToProps)(memo(ResetPassword));