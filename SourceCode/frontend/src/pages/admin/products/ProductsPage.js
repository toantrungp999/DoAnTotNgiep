...
class ProductsPage extends Component {
    ...
    componentDidMount() {
        let { search, category, status, page } = queryString.parse(this.props.location.search);
    ...
    render() {
        ...
        const statusOptions = [{ value: 'all', label: 'Tất cả' }, { value: 'true', label: 'Còn bán' }, { value: 'false', label: 'Ngưng bán' }];
        let loadingComponent;
        let elementData;
        if (loading)
            loadingComponent = <div><Loading /></div>;
        else {
            elementData = products && products.length > 0 ? products.map((product, index) => {
                return <ProductItem key={product._id} index={index} product={product} />
            }) : <tr><td className='no-data' colSpan={7}>Không tìm thấy sản phẩm</td></tr>;
        ...
        return (
            <div className='admin-product-page'>
                ...
                <div className='add-product'>
                    <Link to='/admin/products/create'><i className="fas fa-plus"></i>Thêm mới sản phẩm</Link>
                </div>
                <div className='filter-section'>
                    <SearchProducts history={this.props.history} onChange={this.onChange} search={this.state.search}
                    // searchProducts={this.props.searchProducts} productsReducer={this.props.productsReducer}
                    />
                    <div className=''>
                        <Select name='category' value={this.state.category} firstSubmit={false}
                            label='Loại' placeHolder='Chọn loại' labelWidth='100px'
                    ...
                    <div className=''>
                        <Select name='status' value={this.state.status} firstSubmit={false}
                            label='Trạng thái' placeHolder='Chọn trạng thái' labelWidth='200px'
                            onChange={this.onChange}
                            rules={[]}
                    ...
                    <div>
                        <button className='btn-filter' onClick={this.onSubmit}>Lọc</button>
                    </div>
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        fectchProducts: (key, categoryGroup, category, status, page) => {
            dispatch(adminFectchProductsRequest(key, categoryGroup, category, status, page));
        }
    }
}
