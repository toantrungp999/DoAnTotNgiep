...
import { UserOutlined, PhoneOutlined, CompassOutlined, CalendarOutlined,InfoCircleOutlined } from '@ant-design/icons';
...
class OrderDetailPage extends Component {
    ...
    onConfirmCancel = (reason) => {
        ...
        this.props.orderChangeTypeRequest('admin', this.props.match.params._id, data);
        this.setState({
            showCancelDialog: false,
        })
    }
    ...
    onConfirm = () => {
        this.props.orderChangeTypeRequest('admin', this.props.match.params._id, { type: this.state.type });
        this.setState({
            showConfirmDialog: false,
            type: '',
            message: ''
    ...
    render() {
        ...
        if (this.state.showCancelDialog) {
            confirmCancel = <CancelOrderDialog
                isAdminSide={true}
        ...
        return (
            <div className='order-detail-page'>
                ...
                <div className='order-detail-info'>
                    ...
                    <div className='customer-info-order-type'>
                        <div className='customer-info'>
                            <div><UserOutlined /> {orderInfo.customerInfo.name}</div>
                            <div><PhoneOutlined /> {orderInfo.customerInfo.phoneNumber}</div>
                            <div><CompassOutlined /> {orderInfo.receiveAddress}</div>
                        </div>
                        <div className='order-type'>
                            <div>{orderInfo.orderType}</div>
                            <div>{orderInfo.paymentType}</div>
                            <div>{orderInfo.shipBrand !== '' ? orderInfo.shipBrand : orderInfo.receiveType}</div>
                        </div>
                    </div>
                ...
                <div className='ship-info'>
                    ...
                    <div className='receive-date'><InfoCircleOutlined /> Mã vận đơn: <label>{orderInfo.shipOrderId}</label></div>
                    <div className='receive-date'><CalendarOutlined /> Ngày nhận dự kiến: <label>{getStringDate(orderInfo.expectedReceiveDate)}</label></div>
                </div> : ''}
                ...
                <div className='ship-info'>
                    ...
                    <div className='receive-date'><InfoCircleOutlined /> Mã thanh toán: <label>{orderInfo.paymentId}</label></div>
                </div> : ''}
                <div className='order-detail'>
                    ...
                    <div>
                        {orderDetails.map((orderDetail, index) => {
                            return <OrderDetailItem key={orderDetail._id} index={index}
                                orderDetail={orderDetail} showInstore={orderInfo.status === OrderStatuses.PENDING_APPROVE}
                            ></OrderDetailItem>
                        })}
                    </div>
                    <div className='bottom-section'>
                        <div className="price-section">
                            <div className='total-price'>Tổng giá: {convertNumberToVND(orderInfo.totalPrice)}₫</div>
                            <div className='ship'>
                                {orderInfo.shippingFee !== 0 ? 'Phí vận chuyển: ' + convertNumberToVND(orderInfo.shippingFee) + 'đ' : ''}
                            </div>
                            <div className=''>
                                <label className=''>Tổng tiền: <label className='price'>{convertNumberToVND(orderInfo.total)}₫</label></label>
                            </div>
                        </div>
                    ...
                    <div className='btn-section'>
                        {orderInfo.status === OrderStatuses.PENDING_APPROVE ?
                            <button type="button" className="btn-approve" name='approve' onClick={this.onChangeType}>Duyệt đơn hàng</button> : ''}
                        {orderInfo.status === OrderStatuses.DELIVERING ? <>
                            <button type="button" className="btn-pass" name='pass' onClick={this.onChangeType}>Xác nhận hoàn thành</button>
                            <button type="button" className="btn-fail" name='fail' onClick={this.onChangeType}>Xác nhận thất bại</button>
                        ...
                        {orderInfo.status === OrderStatuses.PENDING_APPROVE || orderInfo.status === OrderStatuses.PENDING_PAY ?
                            <>
                                <button className="btn-cancel" name='cancel' onClick={this.onChangeType}>Hủy đơn hàng</button>
                            </> : ''}
...
export default connect(mapStateToProps, mapDispatchToProps)(memo(OrderDetailPage));