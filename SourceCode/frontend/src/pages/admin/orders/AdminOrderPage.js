...
class OrderPage extends Component {
    ...
    render() {
            ...
            return (
                <div className='order-page'>
                    <div className='order-container'>
                        <div className=''>
                            <OrderSearchByStatus currentStatus={currentStatus} onSwitchStatus={this.onSwitchStatus} />
                        </div>
                        <div className='order-search-bar'>
                            <i className="fas fa-search"></i>
                            <input name='search' placeholder="Tìm kiếm theo Mã đơn hàng, Tên khách hàng hoặc Tên sản phẩm"
                        ...
                        <div>
                            <Paging onFetchData={this.onFetchData} pagingInfo={pageInfo} loading={loading} location={this.props.location} />
                        </div>
                    </div>
                </div>
            );
        ...
        let orderList = orders && orders.length > 0 ? orders.map((order, index) => {
            return (
                <div className='order-page-item' key={order.orderInfo._id} style={{ backgroundColor: '#FFFFFF' }}>
                    <div className='top-bar'>
                        <label className='id'>{order.orderInfo.orderId}</label>
                        <label className='date'>{formatDate(order.orderInfo.date)}</label>
                        <label className='status'>{order.orderInfo.status}</label>
                    </div>
                    <div>
                        {order.orderDetails.map(orderDetail => {
                            return <OrderDetailItem key={orderDetail._id}
                    ...
                    <div className='bottom-section'>
                        <div className='info'>
                            <div className=''><label>{order.orderInfo.orderType}</label></div>
                            <div className=''><label>{order.orderInfo.paymentType}</label></div>
                            <div className=''>
                                <label>{order.orderInfo.shipBrand !== '' ? order.orderInfo.shipBrand : order.orderInfo.receiveType}</label></div>
                        </div>
                        <div className="price-section">
                            <div className='total-price'>Tổng giá: {convertNumberToVND(order.orderInfo.totalPrice)}₫</div>
                            <div className='ship'>
                                {order.orderInfo.shippingFee !== 0 ? 'Phí vận chuyển: ' + convertNumberToVND(order.orderInfo.shippingFee) + 'đ' : ''}
                            </div>
                            <div className=''>
                                <label className='total'>Tổng tiền: <label className='price'>{convertNumberToVND(order.orderInfo.total)}₫</label></label>
                            </div>
                            <div className=''>
                                <Link className='' to={`/admin/orders/${order.orderInfo._id}`} >
