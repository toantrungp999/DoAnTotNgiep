...
import {
    fectchUsersRequest, searchUsersRequest
...
class Users extends Component {
    ...
    onFetchDataFromUrl = () => {
        ...
        if (search)
            this.props.searchUsers(search, paging, 12, true);
        else
            this.props.fectchUsers(paging);
    }
    ...
    render() {
        ...
        if (loading)
            ...
            elementData = users && users.length > 0 ? users.map((user, index) => {
                return <UserItem key={user._id} index={index} user={user} />
            }) : <tr><td className='no-data pt-2 pb-2' style={{ fontSize: '18px', fontWeight: '500' }} colSpan={6}>Không tìm thấy người dùng</td></tr >;
        ...
        return (
            <div>
                ...
                <div className='row'>
                    <SearchUsers history={this.props.history} searchUsers={this.props.searchUsers} usersReducer={this.props.usersReducer} />
                </div>
