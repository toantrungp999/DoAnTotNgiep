...
class DetailUser extends Component {
    ...
    onSubmit = (e) => {
        e.preventDefault();
        this.props.updateStatusRole({ _id: this.state._id, role: this.state.role, status: this.state.status });
    ...
    render() {
        ...
        if (loading)
            ...
            if (this.state.male === undefined)
                male = '------';
            var addresses = this.state.addresses;
            var adressItems = addresses ? addresses.map((address, index) => {
                return <textarea index={index} key={address._id} readOnly value={address.ward + ', ' + address.district + ', ' + address.city} className="form-control mt-3" />
            }) : null;

            var button_submit = loading ? <input type="button" className="btn btn-primary" value="LOADING..." />
                : <input type="submit" className="btn btn-primary" value="XÁC NHẬN" />;

            return (
                <div className="row mt-30">
                    <div className="col-3 mt-30">
                        <img className="avatar-upload mt-30" alt={this.state.name} src={this.state.image} />
                    ...
                    <div className="col-xs-6 col-sm-6 col-md-6 col-lg-6">
                        <form onSubmit={this.onSubmit}>
                            ...
                            <div className="form-group row">
                                ...
                                <div className="col-xs-7 col-sm-7 col-md-7 col-lg-7">
                                    <input readOnly className="form-control" value={this.state.phoneNumber} />
                                </div>
                            </div>
                            <div className="form-group row">
                                <div className="col-xs-3 col-sm-3 col-md-3 col-lg-3">
                            ...
                            <div className="form-group row">
                                ...
                                <div className="col-xs-7 col-sm-7 col-md-7 col-lg-7">
                                    <select className="form-control" name="role" onChange={this.onChange} value={this.state.role}>
                                        ...
                                        <option value={ROLES.CUSTOMER}>Tài khoản bình thường</option>
                                    </select>
                                </div>
                            </div>
...
const mapDispatchToProps = dispatch => ({
    fectchDetailUser: (_id) => { dispatch(fectchDetailUserRequest(_id)) },
    updateStatusRole: (data) => { dispatch(updateStatusRoleRequest(data)) },
})
