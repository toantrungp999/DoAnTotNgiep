...
export const fetchProductRequest = (data) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, "orders/create/fetchCart", "POST", data).then(
      (response) => {
        const type =
          response.status === 0
            ? Types.ORDER_FETCH_PRODUCT_SUCCESS
            : Types.ORDER_FETCH_PRODUCT_FAIL;
...
export const fetchOrdersRequest = (all, status, search, page) => {
  return (dispatch) => {
    ...
    callApiToken(
      dispatch,
      `orders${all}?status=${status}&search=${search}&page=${page}`,
      "GET",
      null
    ).then((response) => {
      const type =
        response.status === 0 ? Types.ORDERS_SUCCESS : Types.ORDERS_FAIL;
      dispatch({ type, payload: response });
    });
  };
};
...
export const fetchOrderRequest = (_id, all) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, `orders${all}/${_id}`, "GET", null).then(
      (response) => {
        const type =
          response.status === 0 ? Types.ORDER_SUCCESS : Types.ORDER_FAIL;
        dispatch({ type, payload: response });
      }
    );
  };
...
export const orderChangeTypeRequest = (path, _id, data) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, `orders/${path}/${_id}`, "PUT", data).then(
      (response) => {
        const type =
          response.status === 0
            ? Types.ORDER_CHANGE_TYPE_SUCCESS
            : Types.ORDER_CHANGE_TYPE_FAIL;
        dispatch({ type, payload: response });
...
export const ShippingFeeRequest = (address) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, `orders/shippingfee`, "POST", address).then(
      (response) => {
        const type =
          response.status === 0
            ? Types.SHIPPING_FEE_SUCCESS
            : Types.SHIPPING_FEE_FAIL;
        dispatch({ type, payload: response });
      }
    );
  };
...
export const fetchStoreAddressesRequest = () => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, "store-address", "GET", null).then((response) => {
      const type =
        response.status === 0
          ? Types.STORE_ADDRESSES_SUCCESS
          : Types.STORE_ADDRESSES_FAIL;
      dispatch({ type, payload: response });
    });
  };
};
...
export const fetchPayRequest = (id) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, `orders/payment/vnpay_url/${id}`, "GET", null).then(
      (response) => {
        const type =
          response.status === 0 ? Types.PAY_URL_SUCCESS : Types.PAY_URL_FAIL;
        dispatch({ type, payload: response });
        if (response.status !== 0)
          dispatch(
            showAlertWithTimeout(
              "Thao tát thất bại, thử lại sau",
              response.message,
              false
            )
          );
