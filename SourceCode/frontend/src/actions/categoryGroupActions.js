...
export const fectchCategoryGroupsRequest = (status = 'all') => {
  return (dispatch) => {
    ...
    callApi(`category-groups/${status}`, 'GET', null).then(response => {
      const type = response.status === 0 ? Types.CATEGORY_GROUPS_SUCCESS : Types.CATEGORY_GROUPS_FAIL;
...
export const createCategoryGroupRequest = (data) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, 'category-groups', 'POST', data).then(response => {
      const type = response.status === 0 ? Types.CATEGORY_GROUP_CREATE_SUCCESS : Types.CATEGORY_GROUP_CREATE_FAIL;
      dispatch({ type, payload: response });
      dispatch(useAlert('Thêm nhóm loại', response.message, response.status === 0));
    });
  };
}
  ...
  return (dispatch) => {
    ...
    callApiToken(dispatch, 'category-groups', 'PUT', data).then(response => {
      ...
      if (response.status === 0) {
        ...
        type = Types.CATEGORY_GROUP_UPDATE_FAIL;
      dispatch({ type, payload: response });
      dispatch(useAlert('Cập nhật nhóm loại', response.message, response.status === 0));
    });
  };
}