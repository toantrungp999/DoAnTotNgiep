...
export const CreateBrandRequest = (data) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, 'brands', 'POST', data).then(response => {
      const type = response.status === 0 ? Types.BRAND_CREATE_SUCCESS : Types.BRAND_CREATE_FAIL;
      dispatch({ type, payload: response });
  ...
  return (dispatch) => {
    ...
    callApiToken(dispatch, 'brands', 'PUT', data).then(response => {
      ...
      if (response.status === 0) {
        ...
        type = Types.BRAND_UPDATE_FAIL;
      dispatch({ type, payload: response });
      dispatch(useAlert('Cập nhật thông tin hãng', response.message, response.status === 0));
    });
  };
}