...
export const createDataColorOptionRequest = (data) => {
  return (dispatch) => {
    callApiToken(dispatch, 'color-options', 'POST', data).then(response => {
      const type = response.status === 0 ? Types.COLOR_OPTION_CREATE_SUCCESS : Types.COLOR_OPTION_CREATE_FAIL;
      dispatch({ type, payload: response });
      dispatch(useAlert('Thêm màu sắc', response.message, response.status === 0));
    });
  };
}
  ...
  return (dispatch) => {
    ...
    callApiToken(dispatch, 'upload-image', 'POST', formData).then(response => {
      if (response.status === 0) {
        ...
        dispatch({ type, payload: response });
        dispatch(showAlertWithTimeout('Thêm màu sắc thất bại', response.message, false));
      }
    });
  };
}
  ...
  return (dispatch) => {
    callApiToken(dispatch, 'color-options', 'PUT', data).then(response => {
      ...
      if (response.status === 0) {
        ...
        type = Types.COLOR_OPTION_UPDATE_FAIL;
      dispatch({ type, payload: response });
      dispatch(useAlert('Chỉnh sửa màu sắc', response.message, response.status === 0));
    });
  };
}
    ...
    if (file) {
      ...
      callApiToken(dispatch, 'upload-image', 'POST', formData).then(response => {
        if (response.status === 0) {
          ...
          dispatch({ type, payload: response });
          dispatch(showAlertWithTimeout('Chỉnh sửa màu sắc thất bại', response.message, false));
        }
      });
    }
    else
...
export const createSizeOptionRequest = (data) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, 'size-options', 'POST', data).then(response => {
      const type = response.status === 0 ? Types.SIZE_OPTION_CREATE_SUCCESS : Types.SIZE_OPTION_CREATE_FAIL;
      dispatch({ type, payload: response });
      dispatch(useAlert('Thêm kích thước', response.message, response.status === 0));
    });
  };
}
  ...
  return (dispatch) => {
    ...
    callApiToken(dispatch, 'size-options', 'PUT', data).then(response => {
      ...
      if (response.status === 0) {
        ...
        type = Types.SIZE_OPTION_UPDATE_FAIL;
      dispatch({ type, payload: response });
      dispatch(useAlert('Chỉnh sửa kích thước', response.message, response.status === 0));
    });
  };
}
...
export const adminFectchQuantityOptionsRequest = (_id) => {
  return (dispatch) => {
    dispatch({ type: Types.QUANTITY_OPTIONS_REQUEST });
    callApi(`quantity-options/admin/productId=${_id}`, 'GET', null).then(response => {
      const type = response.status === 0 ? Types.QUANTITY_OPTIONS_SUCCESS : Types.QUANTITY_OPTIONS_FAIL;
      dispatch({ type, payload: response });
    });
  };
...
export const createQuantityOptionRequest = (data) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, 'quantity-options', 'POST', data).then(response => {
      const type = response.status === 0 ? Types.QUANTITY_OPTION_CREATE_SUCCESS : Types.QUANTITY_OPTION_CREATE_FAIL;
      dispatch({ type, payload: response });
      dispatch(useAlert('Thêm số lượng', response.message, response.status === 0));
    });
  };
}
  ...
  return (dispatch) => {
    ...
    callApiToken(dispatch, 'quantity-options', 'PUT', data).then(response => {
      ...
      if (response.status === 0) {
        ...
        type = Types.QUANTITY_OPTION_UPDATE_FAIL;
      dispatch({ type, payload: response });
      dispatch(useAlert('Chỉnh sửa số lượng', response.message, response.status === 0));
    });
  };
}
///
