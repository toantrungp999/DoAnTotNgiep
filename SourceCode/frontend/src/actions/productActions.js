...
export const searchProductsRequest = (keyword) => {
  return (dispatch) => {
    ...
    callApi(`products/search/key=${keyword}`, 'GET', null).then(response => {
      const type = response.status === 0 ? Types.PRODUCTS_SEARCH_SUCCESS : Types.PRODUCTS_SEARCH_FAIL;
      dispatch({ type, payload: response });
    });
  };
}

export const adminFectchProductsRequest = (search, category, status='all', page) => {
  return (dispatch) => {
    dispatch({ type: Types.PRODUCTS_REQUEST });
    callApi(`products/admin?search=${search}&category=${category}&status=${status}&page=${page}`, 'GET', null).then(response => {
      const type = response.status === 0 ? Types.PRODUCTS_SUCCESS : Types.PRODUCTS_FAIL;
      dispatch({ type, payload: response });
    });
  };
...
export const insertDataProductRequest = (data) => {
  return (dispatch) => {
    callApiToken(dispatch, 'products', 'POST', data).then(response => {
      const type = response.status === 0 ? Types.PRODUCT_CREATE_SUCCESS : Types.PRODUCT_CREATE_FAIL;
      dispatch({ type, payload: response });
      dispatch(useAlert('Thêm sản phẩm', response.message, response.status === 0));
    });
  };
}
...
export const createProductRequest = (files, data) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, 'upload-images', 'POST', formData).then(response => {
      if (response.status === 0) {
        ...
        dispatch(showAlertWithTimeout('Thêm sản phẩm thất bại', response.message, false));
      }
...
export const updateDataProductRequest = (data) => {
  return (dispatch) => {
    callApiToken(dispatch, 'products', 'PUT', data).then(response => {
      ...
      dispatch({ type, payload: response });
      dispatch(useAlert('Cập nhật sản phẩm', response.message, response.status === 0));
    });
  };
}
    ...
    if (files) {
      ...
      callApiToken(dispatch, 'upload-images', 'POST', formData).then(response => {
        if (response.status === 0) {
          ...
          dispatch(showAlertWithTimeout('Cập nhật sản phẩm thất bại', response.message, false));
        }
