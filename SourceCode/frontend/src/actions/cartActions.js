...
export const fectchCartsRequest = () => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, "carts", "GET", null).then((response) => {
      const type =
        response.status === 0 ? Types.CARTS_SUCCESS : Types.CARTS_FAIL;
...
export const reFectchCartsRequest = () => {
  return (dispatch) => {
    callApiToken(dispatch, "carts", "GET", null).then((response) => {
      const type =
        response.status === 0 ? Types.RE_CARTS_SUCCESS : Types.RE_CARTS_FAIL;
      dispatch({ type, payload: response });
    });
  };
};
...
export const createCartRequest = (data) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, "carts", "POST", data).then((response) => {
      const type =
        response.status === 0
          ? Types.CART_CREATE_SUCCESS
          : Types.CART_CREATE_FAIL;
      dispatch({ type, payload: response });
      dispatch(
        useAlert(
...
export const updateCartRequest = (data) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, "carts", "PUT", data).then((response) => {
      ...
      dispatch({ type, payload: response });
      // dispatch(useAlert('Thêm sản phẩm vào giỏ hàng', response.message, response.status === 0));
    });
  };
};
...
export const updateTypeCartRequest = (data) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, "carts/update-type", "PUT", data).then(
      (response) => {
        let type = null;
        if (response.status === 0) {
          type = Types.CART_UPDATE_TYPE_SUCCESS;
          response.data = { cart: response.data, cartId: data.cartId };
        } else type = Types.CART_UPDATE_TYPE_FAIL;
        dispatch({ type, payload: response });
        dispatch(
          useAlert("Chỉnh sửa", response.message, response.status === 0)
        );
      }
    );
  };
  ...
  return (dispatch) => {
    ...
    callApiToken(dispatch, `carts/${data}`, "DELETE", null).then((response) => {
      ...
      if (response.status === 0) {
        ...
        response.data = data;
      } else type = Types.CART_DELETE_FAIL;
      dispatch({ type, payload: response });
      dispatch(
        useAlert("Xóa giỏ hàng", response.message, response.status === 0)
      );
    });
  };
};
