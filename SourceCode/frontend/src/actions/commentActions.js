...
export const fectchCommentsRequest = (productId, lengthCmt = 5) => {
  return (dispatch) => {
    ...
    callApi(`comments/${productId}&pagesize=${lengthCmt}`, "GET", null).then(
      (response) => {
        const type =
          response.status === 0 ? Types.COMMENTS_SUCCESS : Types.COMMENTS_FAIL;
...
export const createCommentRequest = (data, dataUser) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, "comments", "POST", data).then((response) => {
      ...
      } else type = Types.COMMENT_CREATE_FAIL;
      dispatch({ type, payload: response });
    });
  };
};
...
export const updateCommentRequest = (data) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, "comments", "PUT", data).then((response) => {
      const type =
        response.status === 0
          ? Types.COMMENT_UPDATE_SUCCESS
          : Types.COMMENT_UPDATE_FAIL;
      dispatch({ type, payload: response });
    });
  };
};
  ...
  return (dispatch) => {
    ...
    callApiToken(dispatch, "comments", "DELETE", data).then((response) => {
      ...
      if (response.status === 0) {
        ...
        response.data = data;
      } else type = Types.COMMENT_DELETE_FAIL;
      dispatch({ type, payload: response });
    });
  };
};
...
export const createReplyRequest = (data) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, "comments/replies", "POST", data).then(
      (response) => {
        const type =
          response.status === 0
            ? Types.REPLY_CREATE_SUCCESS
            : Types.REPLY_CREATE_FAIL;
        dispatch({ type, payload: response });
      }
    );
  };
...
export const updateReplyRequest = (data) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, "comments/replies", "PUT", data).then((response) => {
      const type =
        response.status === 0
          ? Types.REPLY_UPDATE_SUCCESS
          : Types.REPLY_UPDATE_FAIL;
      dispatch({ type, payload: response });
    });
  };
};
...
export const deleteReplyRequest = (data) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, "comments/replies", "DELETE", data).then(
      (response) => {
        let type = null;
        if (response.status === 0) {
          type = Types.REPLY_DELETE_SUCCESS;
          response.data = data;
        } else type = Types.REPLY_DELETE_FAIL;
        dispatch({ type, payload: response });
      }
    );
  };
