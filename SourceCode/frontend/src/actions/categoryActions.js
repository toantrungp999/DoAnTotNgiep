...
export const fectchCategoriesRequest = (status = 'all') => {
  return (dispatch) => {
    ...
    callApi(`categories/${status}`, 'GET', null).then(response => {
      const type = response.status === 0 ? Types.CATEGORIES_SUCCESS : Types.CATEGORIES_FAIL;
...
export const createCategoryRequest = (data) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, 'categories', 'POST', data).then(response => {
      const type = response.status === 0 ? Types.CATEGORY_CREATE_SUCCESS : Types.CATEGORY_CREATE_FAIL;
      dispatch({ type, payload: response });
      dispatch(useAlert('Thêm loại sản phẩm', response.message, response.status === 0));
    });
...
export const updateCategoryRequest = (data) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, 'categories', 'PUT', data).then(response => {
      ...
      dispatch({ type, payload: response });
      dispatch(useAlert('Cập nhật loại sản phẩm', response.message, response.status === 0));
    });
  };
}