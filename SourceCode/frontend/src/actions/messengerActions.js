...
export const initialization = () => {
  return (dispatch) => {
    socket.on("sendMessageToBot", (response) => {
      dispatch(reciverMessage(response));
    });
    socket.on("sendMessageToUser", (response) => {
      dispatch(reciverMessage(response));
    });
    socket.on("login", (response) => {
      receiveMessage(response);
    });
    socket.on("updateMessengerCheck", (response) => {
      dispatch(updateMessengerSuccess(response));
    });
    socket.on("sokectError", (response) => {
      dispatch(sokectError(response));
    });
    socket.on("senMessageFail", (response) => {
      dispatch(senMessageFail(response));
    });
  };
};
...
export const fectchMessengersRequest = () => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, "messengers", "GET", null).then((response) => {
      const type =
        response.status === 0
          ? Types.MESSENGERS_SUCCESS
          : Types.MESSENGERS_FAIL;
...
export const fectchMessagesRequest = (messengerId, pageSize, currentPage) => {
  return (dispatch) => {
    ...
    callApiToken(
      dispatch,
      `messages/messengerId=${messengerId}&pageSize=${pageSize}&currentPage=${currentPage}`,
      "GET",
      null
    ).then((response) => {
      const type =
