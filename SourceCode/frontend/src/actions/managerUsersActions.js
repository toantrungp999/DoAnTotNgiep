...
export const fectchUsersRequest = (page = 1) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, `users/page=${page}`, 'GET', null).then(response => {
      const type = response.status === 0 ? Types.USERS_SUCCESS : Types.USERS_FAIL;
...
export const searchUsersRequest = (keyword, page = 1, pageSize, isSearch = false,) => {
  return (dispatch) => {
    if (isSearch)
      dispatch({ type: Types.USERS_REQUEST });
    else
      dispatch({ type: Types.USERS_SEARCH_REQUEST, payload: { keyword } });
    callApiToken(dispatch, `users/searchString=${keyword}&page=${page}&pagesize=${pageSize}`, 'GET', null).then(response => {
      let type = null;
      if (isSearch)
...
export const fectchDetailUserRequest = (_id) => {
  return (dispatch) => {
    ...
    callApiToken(dispatch, `users/${_id}`, 'GET', null).then(response => {
      const type = response.status === 0 ? Types.USER_DETAIL_SUCCESS : Types.USER_DETAIL_FAIL;
      dispatch({ type, payload: response });
    });
  };
}
  ...
  return (dispatch) => {
    ...
    callApiToken(dispatch, 'users', 'PUT', data).then(response => {
      ...
      if (response.status === 0) {
        ...
        type = Types.USER_DETAIL_FAIL;
      dispatch({ type, payload: response });
      dispatch(useAlert('Cập nhật quyền người dùng', response.message, response.status === 0));
    });
  };
}