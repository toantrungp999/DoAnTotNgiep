...
export const getGeocodeRequest = (data) => {
  return (dispatch) => {
    ...
    callApi(`location/geocode?city=${data.city}&district=${data.district}&ward=${data.ward}&streetOrBuilding=${data.streetOrBuilding}`, 'GET').then(response => {
      console.log(response);
      const type = response.status === 0 ? Types.GEOCODE_SUCCESS : Types.GEOCODE_FAIL;
      dispatch({ type, payload: response });
    });
  };
}
...
export const getLocationRequest = (data)=>{
  return (dispatch) => {
    ...
    callApi(`location/location?lat=${data.lat}&lng=${data.lng}`, 'GET').then(response => {
      console.log(response);
      const type = response.status === 0 ? Types.LOCATION_SUCCESS : Types.LOCATION_FAIL;
      dispatch({ type, payload: response });
    });
  };
}