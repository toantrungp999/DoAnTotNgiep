    ...
    rules.map(rule => {
        ...
        switch (key) {
            ...
            case 'email':
                ...
                break;
            case 'equal':
                if (value.toString().trim() !== rule.equal.toString().trim()) valid = false;
                break;
            case 'biggerDate':
                if (rule.biggerDate && Date.parse(new Date(value)) <= Date.parse(new Date(rule.biggerDate))) valid = false;
                break;
            default:
                break;
        }
