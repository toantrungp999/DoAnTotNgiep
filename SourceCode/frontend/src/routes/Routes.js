    ...
    render() {
        ...
        return (
            <Router history={history}>
                <Switch>
                    {role && role !== ROLES.CUSTOMER ? <Route path="/admin" component={HeaderAdmin} /> : ''}
                    <Route path="/" component={HeaderCustomer} />
                </Switch>
                <div className="body-background">
                    {userInfo && action === Types.CREATE_NEW_MESSAGE ? <CreateMessage role={role} /> : null}
                    {userInfo && action === Types.OPEN_DETAIL_MESSENGER ? <DetailMessage userId={userInfo._id} to={to} index={index} /> : null}
                    <Alert />
                    <Switch>
                        ...
                        {role && role !== ROLES.CUSTOMER ? <Route path="/admin" component={HomeAdmin} /> : ''}
                        {role ? <Route path="/account" component={NavCustomer} /> : null}
                        {role ? <Route path="/cart" component={BasketCartPage} /> : null}
                        {role ? <Route path="/orders/create" exact component={CreateOrderPage} /> : null}
                        {role ? <Route path="/orders" exact component={NavCustomer} /> : null}
                        {role ? <Route path="/orders/:_id" exact component={NavCustomer} /> : null}
                ...
                <Switch>
                    {role && role !== ROLES.CUSTOMER ? <Route path="/admin" component={FooterAdmin} /> : ''}
