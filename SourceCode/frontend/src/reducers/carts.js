  ...
  switch (action.type) {
    ...
    case Types.CART_CREATE_SUCCESS:
      ...
      if (state.carts !== undefined) {
        ...
        if (index >= 0) state.carts[index] = action.payload.data;
        else state.carts.push(action.payload.data);
      }
      return { ...state };
    ...
    case Types.CART_UPDATE_TYPE_SUCCESS:
      ...
      index = findIndexById(state.carts, cart._id);
      if (index >= 0) state.carts[index] = cart;
    ...
    case Types.CART_CLEAR_STATE:
      return { carts: state.carts, loading: false };
    default:
      return state;
  }
}
