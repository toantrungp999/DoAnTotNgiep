...
class SizeOptionItem extends Component {
    ...
    render() {
        ...
        let size = '';
        let index = sizeOptions ? findIndexById(sizeOptions, this.props.quantityOption.sizeId) : -1;
        if (index > -1)
            size = sizeOptions[index].size;
        ...
        if (this.state.isEditting) {
            return (
                <tr>
                    ...
                    <td>
                        <div className="row">
                            <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                                <input className={"form-control" + (errors.quantity.length > 0 ? ' invalid-input' : '')} name="quantity" value={this.state.quantity} onChange={this.onChange} type="number" />
                            </div>
                        </div>
                    ...
                    <td>
                        <div>
                            <button type="button" onClick={this.onSubmit} className="btn-icon btn btn-primary mb-1"><i className="fa fa-save"></i></button>
                        </div>
                        <div>
