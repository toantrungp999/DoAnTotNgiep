    ...
    render() {
        ...
        return (
            <tr className='first-row'>
                <td>
                    <input className={"form-control" + ((firstSubmit && errors.size.length > 0) ? ' invalid-input' : '')} name="size" value={this.state.size} onChange={this.onChange} />
                    <div className='invalid-message text-center'>{firstSubmit ? errors.size : ''}</div>
                </td>
                <td>
                    <button className='btn-icon btn btn-primary' type="button" style={{ border: 'none' }} id="add-config" onClick={this.onSubmit}>
