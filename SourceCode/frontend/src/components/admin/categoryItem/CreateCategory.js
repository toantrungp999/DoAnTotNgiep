...
class CreateCategory extends Component {
    ...
    render() {
        ...
        return (
            <tr>
                <td>
                    <input name="name" value={this.state.name} onChange={this.onChange} className="form-control" />
                    {this.state.errors.name.length > 0 && <span className='text-danger'>{this.state.errors.name}</span>}
                </td>
                <td>
                    <select className="form-control" name="categoryGroupId" onChange={this.onChange} value={this.state.categoryGroupId} >
                        <option value={-1}>Chọn nhóm loại sản phẩm</option>
                        {categoryGroupsOption}
                    </select>
                    {this.state.errors.categoryGroupId.length > 0 && <span className='text-danger'>{this.state.errors.categoryGroupId}</span>}
                </td>
                <td><input type='checkbox' readOnly checked /></td>
                <td>
                    <button type="button" className="btn-icon btn btn-primary" onClick={this.onSubmit}>
