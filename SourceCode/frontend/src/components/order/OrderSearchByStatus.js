...
class OrderSearchByStatus extends Component {
    ...
    render() {
        let statusBar = [];
        statusBar.push(
            <button key={-1} type="button" className={this.props.currentStatus === 'Tất cả' ? 'active' : ''}
                 onClick={() => this.onSwitchStatus("Tất cả")}>Tất cả</button>
        )
        for (const [index, value] of Object.entries(OrderStatuses)) {
            ...
            statusBar.push(
                <button key={index} className={active} type="button" onClick={() => this.onSwitchStatus(value)}>{dislayValue}</button>
            )
        }

        return (
            <>
                <ul className="order-search-by-status">
                    {statusBar}
                </ul>
            </>
        )
