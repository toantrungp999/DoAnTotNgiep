...
import {
    createOrderRequest, ShippingFeeRequest
...
import { fetchCitiesRequest, fetchDistrictsRequest, mapClearState, getGeocodeRequest, getLocationRequest } from '../../actions/locationActions';
...
class OrderType extends React.Component {
        ...
        switch (name) {
            case 'orderType':
                if (value === OrderTypes.ONLINE_ORDER.ORDER_TYPE) {
                    this.setState({ name: this.props.userInfoReducer.userInfo.name, phoneNumber: this.props.userInfoReducer.userInfo.phoneNumber ? this.props.userInfoReducer.userInfo.phoneNumber : '' });
                } else {
    ...
    render() {
        ...
        let { addressId, newAddress, orderType, paymentType, receiveType, name, phoneNumber, shipId, shipDialog, firstSubmit } = this.state
        ...
        for (const [index, value] of Object.entries(OrderTypes)) {
            ...
            if (value.ORDER_TYPE === OrderTypes.IN_STORE_ORDER.ORDER_TYPE) {
                if (role === 'ADMIN' || role === 'SUPER_ADMIN') {
                    OrderTypeOptions.push({ value: value.ORDER_TYPE, label: value.ORDER_TYPE });
                }
            } else {
                OrderTypeOptions.push({ value: value.ORDER_TYPE, label: value.ORDER_TYPE });
            }
        ...
        if (receiveType === OrderTypes.ONLINE_ORDER.RECEIVE_TYPE.SHIP) {
            addressOptions = userAddresses ? userAddresses.map((address, index) => {
                return ({
        ...
        return (
            <div className='order-type-component'>
                <div className='order-type-section' >
                    <Select name='orderType' value={orderType} firstSubmit={firstSubmit}
                        label='Hình thức' labelWidth='200px'
                        setValidate={this.setValidate} onChange={this.onChange}
                        rules={[{ require: true, message: 'Chưa nhập' }]}
                        options={OrderTypeOptions}
                    />
                    {orderType === OrderTypes.IN_STORE_ORDER.ORDER_TYPE ? <>
                        <Label label='Nhân viên' labelWidth='200px'
                            value={this.props.userInfoReducer.userInfo.name} />
                    </> : ''}
                    <Input name='name' value={name} firstSubmit={firstSubmit}
                        label='Tên khách hàng' labelWidth='200px'
                        setValidate={this.setValidate} onChange={this.onChange}
                        rules={[{ require: true, message: 'Chưa nhập' }, { max: 255, message: 'Nhập dưới 255 ký tự' }]} />
                    <Input name='phoneNumber' value={phoneNumber} firstSubmit={firstSubmit}
                        label='Số điện thoại' labelWidth='200px'
                        setValidate={this.setValidate} onChange={this.onChange}
                        rules={[{ require: true, message: 'Chưa nhập' }, { phoneNumber: true, message: 'Không phải số điện thoại' }]} />

                    <Select name='paymentType' value={paymentType} firstSubmit={firstSubmit}
                        label='Thanh toán' placeHolder='Chọn hình thức thanh toán' labelWidth='200px'
                        setValidate={this.setValidate} onChange={this.onChange}
                        rules={[{ require: true, message: 'Chưa nhập' }]}
                        options={PaymentTypeOptions}
                    />

                    <Select name='receiveType' value={receiveType} firstSubmit={firstSubmit}
                        label='Nhận hàng' placeHolder='Chọn hình thức nhận hàng' labelWidth='200px'
                        setValidate={this.setValidate} onChange={this.onChange}
                        rules={[{ require: true, message: 'Chưa nhập' }]}
                        options={ReceiveTypeOptions}
                    />
                    {receiveType === OrderTypes.ONLINE_ORDER.RECEIVE_TYPE.SHIP ?
                        <Select name='address' value={addressId} firstSubmit={firstSubmit}
                            label='Địa chỉ' placeHolder='Chọn địa chỉ giao hàng' labelWidth='200px'
                            setValidate={this.setValidate} onChange={this.onChange}
                            rules={[{ require: true, message: 'Chưa nhập' }]}
                            options={addressOptions}
                        /> : receiveType === OrderTypes.ONLINE_ORDER.RECEIVE_TYPE.IN_STORE ?
                            <Label label='Cửa hàng' labelWidth='200px'
                                value={storeAddress.streetOrBuilding + ', ' + storeAddress.ward + ', '
                                    + storeAddress.district + ', ' + storeAddress.city} />
                            : ''}
                    {addressId === '-100' ?
                    ...
                    <div>
                        <div >
                            {shipId !== '-2' ? <div className='form-field ship'>
                                <label style={{ width: '200px' }}>Giao hàng</label>
                                <div>
                                    <div className='selected-ship-section' onClick={this.openChooseShip}>
                                        <div className='ship-brand'>{shipId !== '-1' ? shipInfos[shipId].name : 'Chọn phương thức'}</div>
                                        <div className='ship-info'>
                                            <div className='ship-fee'>{shipId !== '-1' ? (convertNumberToVND(shipInfos[shipId].shippingFee) + 'đ') : ''}</div>
                                            <div className='receive-date'>{shipId !== '-1' ? ('Nhận ' + getStringDate(shipInfos[shipId].date * 1000)) : ''}</div>
                                        </div>
                                    </div>
                                    {firstSubmit && shipId === '-1' ? <div className='invalid-message'><WarningOutlined /> Chưa chọn</div> : ''}
                                </div>

                            </div> : ''}
                            {shipDialog ? <ChooseShip shipInfos={shipInfos}
                                shipId={shipId} closeChooseShip={this.closeChooseShip} onChooseShip={this.onChooseShip} /> : ''}
                ...
                <div className='submit-section'>
                    ...
                    <div className='total-section'>
                        ...
                        <span className='price'>{(shipId !== '-2' && shipId !== '-1')
                            ? convertNumberToVND(total + shipInfos[shipId].shippingFee) + '₫'
                            : convertNumberToVND(total) + '₫'}</span>
                    </div>
                    <div>
                        {this.props.createSuccess === null ? <button className="btn-order" >ĐANG TẢI...</button>
                            : <button className="btn-order" onClick={this.CreateOrder}>ĐẶT HÀNG</button>}
                    </div>
