...
class ChooseShip extends Component {
    ...
    render() {
        ...
        const shipOptions = shipInfos === null ? <Loading3QuartersOutlined spin /> : shipInfos === 'fail' ? <div>Thất bại, vui lòng thử lại sau</div> :
            shipInfos.map((info, index) => {
                return (<div key={index} className={'ship-option' + (Number(shipId) === index ? ' active' : '')}
                    onClick={(e) => { this.onChooseShip(e, index) }}>
                    <div className='ship-brand'>{info.name}</div>
                    <div className='ship-info'>
                        <div className='ship-fee'>{convertNumberToVND(info.shippingFee)}₫</div>
                        <div className='receiveDate'>Nhận {getStringDate(info.date * 1000)}</div>
                    </div>
                </div>)
            });
        return (
            <div className={'dialog-black-backgound' + (this.state.closing ? ' closing' : '')} onClick={this.onClosing}>
