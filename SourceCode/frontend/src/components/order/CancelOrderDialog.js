...
class CancelOrderDialog extends Component {
    ...
    onConfirm = () => {
        ...
        if (reason !== '') {
            var cancelReason = this.props.isAdminSide ? 'Người bán: ' : 'Khách hàng: ';
    ...
    render() {
        ...
        return (
            <div className="cancel-order-dialog" >
                ...
                <div className="cancel-order-dialog-container" onClick={e => e.preventDefault()}>
                    <h5 className="dialog-title">Xác nhận hủy đơn hàng</h5>
                    <div>
                        <Select name='reason' value={this.state.reason} firstSubmit={firstSubmit}
                            label='Lý do' labelWidth='' vertical={true} placeHolder='Chọn lý do'
                        ...
                        {this.state.reason === CancelReasons.BY_ADMIN.OTHER ? <>
                            <Input name='otherReason' value={this.state.otherReason} firstSubmit={firstSubmit}
                                label='Lý do khác' labelWidth='' vertical={true}
                                setValidate={this.setValidate} onChange={this.onChange} placeHolder='Nhập lý do'
                                rules={[{ max: 255, message: 'Nhập dưới 255 ký tự' }]} />

                        </> : ''}
                    </div>
                    <div className="btn-section">
                        <button className="btn-close" onClick={this.onClose} >Trở lại</button>
                        <button className="btn-confirm" onClick={this.onConfirm}>Hủy đơn hàng</button>
                    </div>
