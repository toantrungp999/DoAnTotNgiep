...
class Messengers extends Component {
    ...
    render() {
        ...
        let notSeen = 0;
        const messengerItems = messengers ? messengers.map((messenger, index) => {
            ...
            return <MessengerItem currentUserSend={currentUserSend} isMeSend={isMeSend} openDetailMessenger={this.openDetailMessenger} index={index} key={messenger._id} messenger={messenger} userId={userInfo._id} />
        }) : null;

        return (<div>
            <Link to="#" onClick={(e) => { e.preventDefault(); this.props.onViewMessengers(); }} className="nav-link messenger">
                <i className="fab icon fa-facebook-messenger"></i>{notSeen !== 0 ? <span className='num'>{notSeen}</span> : null}
            </Link>
            {
                this.props.viewMessengers ?
                    <div className="messengers-dialog">
                        <div className="row">
                            ...
                            <div className="col-2">
                                <i className="messenger-create-message far fa-edit" onClick={this.createNewMessage}></i>
                            </div>
                        </div>
                        ...
                        <div className="row" style={{ marginBottom: '10px', marginTop: '10px' }}>
                            <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                                <div className="form-group">
                                    <input className="form-control search-messenger" name="search" onChange={this.onChange} type="text" placeholder="Tìm kiếm" />
                                </div>
                            </div>
                        </div>
                        {messengerItems}
