...
//     const { loading, categories } = this.props.categoriesReducer;
//     if (!loading) {
//       let count = 0;
//       const linksType = categories ?
//         categories.map((category, index) => {
//           count++;
//           const path = brandId ? `/brand=${brandId}/category=${category._id}` : `/category=${category._id}`;
...
//               return <li style={{ width: '112px' }}><Link className={active} to={path}>{category.name}</Link></li>
//             }}
//           />
//         }) : [];
//       if (!this.state.viewMore)
//         linksType.push(<Link key="read-more-searchByType" className="nav-link search-nav-link" to="#" onClick={(e) => { e.preventDefault(); this.setState({ viewMore: true }); }}>Xem thêm</Link>)
//       return (
