                ...
                <Collapse in={show}>
                    <div className="header-search">
                        <input id="search-keyword"
                            type="text" value={this.state.search} name="search"
                            onChange={this.onChange} aria-label="Bạn tìm gì..." onKeyDown={this.handleKeyDown}
                            placeholder="Bạn tìm gì..." autoComplete="off" maxLength="50" />
                        <button type="button" aria-label="tìm kiếm" onClick={this.props.closeSearch}><i className="fas fa-times"></i></button>

                    </div>
                    <Suggestions results={searchProducts} closeSearch={this.props.closeSearch} />
                </Collapse>
            </div >
        )
    }
