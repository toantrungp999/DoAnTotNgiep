...
class Input extends Component {
    ...
    render() {
        ...
        const { firstSubmit, name, value, label, labelWidth, type, placeHolder, vertical } = this.props;
        const invalid = (message.length > 0 && (firstSubmit === true || change === true)) ? true : undefined;

        return (
            <div className={'form-field' + (invalid ? ' invalid' : '') + (vertical ? ' vertical' : '')} >
                <label style={{ width: labelWidth }}>{label}</label>
                <div>
                    <input name={name} value={value} onChange={this.onChange} type={type ? type : 'text'} placeholder={placeHolder ? placeHolder : ''} />
                    {invalid ? <div className='invalid-message'><WarningOutlined /> {message}</div> : ''}
                </div>
