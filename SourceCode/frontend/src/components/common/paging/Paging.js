...
class Paging extends Component {
    ...
    render() {
                ...
                if (i === pagingInfo.currentPage)
                    paging.push(
                        <li key={i} className="page active">
                            <Link className="" to="#" onClick={(e) => { e.preventDefault(); }}>{i}
                                <span className="sr-only">(current)</span>
                            </Link>
                        </li>
                    )
                else {
                    if(i===1||i===pagingInfo.totalPage || Math.abs(Math.abs(i - pagingInfo.currentPage)<3)){
                        paging.push(
                            <li key={i} className="page">
                                <Link className="" to="#" onClick={(e) => { e.preventDefault(); this.onClickPage(i) }}>{i}</Link>
                            </li>
                        )
                    }
        ...
        const pagingElement = loading ? <div></div> :
            <nav className="paging-component">
                <ul className="">
                    {pagingInfo && pagingInfo.currentPage !== 1 ?
                        <li className="page">
                            <Link className="" to="#" onClick={(e) => { e.preventDefault(); this.onPrev() }} aria-label="Previous">
                                {'<'}
                            </Link>
                        </li> : null
                    }
                    {paging}
                    {pagingInfo && (pagingInfo.currentPage === pagingInfo.totalPage || pagingInfo.totalPage === 0) ? null :
                        <li className="page">
                            <Link className="" to="#" onClick={(e) => { e.preventDefault(); this.onNext() }} aria-label="Next">
                                {'>'}
                            </Link>
                        </li>
                    }
